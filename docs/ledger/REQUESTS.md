...
| R160 | 2026-08-15 | `perf` | | `open` | **[walkthrough]** كل مسار ~١٣٫٤ ثانية حتى سكون الشبكة — تقدّم في R268: ثلاثةُ مسارات كانت **لا تسكن أبدًا** صارت تسكن دون ثانية؛ والباقي طلبُ `sdk.minepi.com` وحدَه، ويحتاج قياسًا على شبكةٍ حقيقية |
...
### R272 — دخول Pi التلقائي: أُنجز

الدخول التلقائي كان **مكتوبًا وسليمًا**، لكنه لم يُنادَ قطّ لبعض الرواد.
حلقة مفرغة:

```
surface = pi-browser  ⟸ window.Pi موجود
window.Pi موجود       ⟸ التطبيق حمّل الـSDK
التطبيق يحمّل الـSDK   ⟸ surface = pi-browser
```

ومتصفّح Pi **لا يحقن `window.Pi` بنفسه** — التطبيق يحمّله. فبقي الاعتماد على
وسمٍ في الـuser agent، ومتصفّح Pi لا يضمنه (قائمة الأوسمة نفسها كُتبت لأنّ
الوسم تغيّر من قبل). فرائدٌ لا يحمل متصفّحه الوسم كان **يُعرض عليه نموذج
البريد داخل متصفّح Pi** — عين العطل الذي كُتبت القائمة لتفاديه.

**الحلّ: قدرةٌ تُختبَر لا سلسلةٌ تُطابَق.** `lib/auth/pi-probe.ts` يحمّل
الـSDK **مرّة واحدة بعد أوّل رسم** (على `requestIdleCallback`) ويسأل Pi عن
الهويّة. المصافحة تنجح داخل متصفّح Pi ولا تستقرّ في غيره — فنجاحها **برهانٌ**
لا تخمين. ويطلب `username` فقط: طلب `payments` هنا يضع موافقة دفعٍ أمام من
فتح صفحةً لا أكثر.

وما بطل بذلك مذكورٌ صراحةً: بعد أن صرنا نحمّل الـSDK على الويب المفتوح، لم
يعد وجود `window.Pi` دليلًا على شيء.

### R273 — إخفاء الاشتراك: أُنجز

`SUBSCRIPTION_VISIBLE` — مخفيّ افتراضيًّا، **منفصل عن `TIERS_ENFORCED`**.
والفصل مقصود: الإنفاذ يجيب «هل يتوقّف المجّاني عند عشرين تحقيقًا»، وهذا يجيب
«هل يرى الزائر سعرًا أصلًا». دمجهما كان سيترك اللوحة معروضة بلا إنفاذ — أي
دعوةً للدفع مقابل ما يملكه كلّ زائر أصلًا.

**مخفيّ لا محذوف:** الخطط والأسعار ومسار الدفع بـPi واختباراتها كما هي،
فالعودة متغيّرٌ واحد لا إعادة بناء.

---

## R274 — «افحص قاعدة البيانات والسياسات»

فحصتُها. **١٩ من ٢٤ جدولًا كانت بلا أمن على مستوى الصفّ (RLS)، وصفر سياسات
في المشروع كلّه.** الخمسة المحميّة هي فقط ما أُضيف في الهجرات ٠٠٠٩–٠٠١٣، حين
بدأت الممارسة؛ والتسعة عشر الأقدم بقيت مكشوفة.

**ولماذا هذا ليس شكليًّا على Supabase:** واجهة PostgREST مفتوحة افتراضيًّا،
ومفتاح anon **عموميّ بالتصميم** — يُشحن إلى كلّ متصفّح ويُفترض به ذلك. والشيء
الوحيد بينه وبين جدولٍ في `public` هو RLS. وكان مكشوفًا:

```
credentials          تجزئات كلمات السرّ
verification_codes   رموز تسجيل واستعادة حيّة
users · email_followers   بيانات شخصية يحميها §3 من الميثاق
investigations · scans · evidence   كلّ ما خزّنه المنتج
alerts · monitors · entities · entity_links · ontology_*
radar_findings · calibration_claims · suggestions · sources
source_health_daily · blobs
```

**والتفعيل صفر خطر على التطبيق:** يتّصل بـ`postgres-js` عبر `DATABASE_URL` —
اتّصالٌ مباشر بدور المالك **يتجاوز RLS بحكم التعريف**. ولم يستعمل يومًا عميل
Supabase أو مفتاح anon (بحثٌ في الشجرة كلّها عن `SUPABASE_ANON` و
`NEXT_PUBLIC_SUPABASE` و`createClient` لا يعيد شيئًا).

**وبلا سياسات عمدًا:** السياسة تمنح وصولًا، والمنح الصحيح هنا **لا شيء**.
ميزةٌ مستقبلية تحتاج قراءةً من المتصفّح تأخذ سياسةً تُكتب لحالتها وتُناقَش،
لا غيابًا مورَّثًا.

واختبارٌ يمنع عودة الثغرة: أيّ جدولٍ جديد بلا RLS **يُسقط الاختبارات** — في
المخطّط وفي الهجرات معًا، لأنّ حمايةً في أحدهما دون الآخر تعني قاعدتين
تختلفان بالضبط حيث لا يخطر لأحد أن يتفقّد.

## R275 — «تصميمنا الحالي لصفحة الكرة غير مقبول نهائيًّا»

نصّ الطلب، حرفيًّا:

> تفحص المواقع الاخرى خاصة موقع خاصة هذا وسترى الكثير من الاشياء المفقودة في
> مشروعنا والترتيب والتقنيات طور عنه خاصة التصميم تصميمنا الحالي لكل صفحة
> الكرة غير مقبول نهائيا

مع اختصارٍ مرفَق إلى **World Monitor** (`worldmonitor.app`).

**الحالة:** الجزء الأوّل منفَّذ ومقيس — عيبان في التصميم أُصلحا. والباقي
مسجَّل كأربع مهامّ (R275a–R275d) لأنّه بناءٌ لا ترقيع.

**القيد الذي لا يُخترق:** World Monitor مرخَّص **AGPL‑3.0**، وهو أشدّ نسخ
الحقوق المتروكة شيوعًا: لمس شفرتهم يجرّ منتجنا كلّه إلى الفتح. فالقاعدة —
المكتوبة في §2.8 من الميثاق ومكرَّرة هنا لأنّها تُنسى تحت ضغط «انسخ ما يعمل» —
هي: **نُدرس المعمار، ونبني بأنفسنا. لا سطر واحد من شفرتهم.**


### ما قِيس فعلًا، لا ما شعرتُ به

عيبان، وكلاهما عدد لا ذوق: شيئان يفترض أن يتّفقا ولم يتّفقا.

**١ — الترويسة لا تشارك الصفحة عرضها.** كانت `max-w-2xl` (٦٧٢px) من زمنٍ كان
المنتج فيه عمودَ قراءةٍ واحدًا، والمحتوى كبر إلى `88rem`:

| العرض | الترويسة | المحتوى | الفارق بين الحافّتين |
|---|---|---|---|
| ١٤٤٠px | ٦٧٢px عند x=384 | ٧٨٤px عند x=304 | **٨٠px** |
| ١٩٢٠px | ٦٧٢px عند x=624 | ١٠٤٠px عند x=416 | **٢٠٨px** |

الآن ثابتٌ واحدٌ يقرؤه الملفّان (`lib/shell-width.ts`)، واختبارٌ يسقط إن كتب
أحدهما عرضه بنفسه. القياس بعد الإصلاح: الحافّة نفسها عند ٣٩٠ و٧٦٨ و١٤٤٠ و١٩٢٠.

**٢ — الكرة توقّفت عن النموّ عند ٥٧٤px.** لوحة الخريطة كانت مسقوفة بـ`38rem`
والرفّ يأخذ الباقي، فكلّ بكسل تضيفه شاشةٌ أكبر كان يذهب إلى اللوحة التي لا
تحتاجه:

| العرض | لوحة الكرة | الرفّ | نصّ الرفّ |
|---|---|---|---|
| ١٢٨٠ | ٥٠٢px | ٥٠٥px | ٤١٥ حرفًا |
| ١٤٤٠ | ٥٧٤px | ٥٩٢px | ٤١٥ حرفًا |
| ١٩٢٠ | **٥٧٤px** | ١٠٧٢px | ٤١٥ حرفًا |
| ٢٥٦٠ | **٥٧٤px** | ١٧١٢px | ٤١٥ حرفًا |

على شاشة ٢٥٦٠ كان ثلثا العرض يحملان أربعمئة حرف، وواجهة المنتج الرئيسة عند
٥٧٤px. السقف انتقل إلى الرفّ — وهو اللوحة التي لها عرضٌ صحيحٌ طبيعيّ — والخريطة
تأخذ الباقي. القياس بعد الإصلاح: ٥٩٠ ← ٧٥٠ ← ١١٣٤ ← ١٧٧٤.

وأمران تابعان: الرفّ الفارغ كان **يرسم فراغًا صامتًا** فصار يقول أيّ الأسباب
الثلاثة هذا ويفصل «مصدرٌ أجاب بلا شيء» عن «مصدرٌ لم يُجب»؛ والكرة كانت مُلصَقة
على بطاقةٍ بيضاء في السمة الفاتحة مع أنّ كلّ لونٍ ترسمه داكنٌ ثابت، فصارت
أرضيّتها فضاءً كلوحتها.

### وماذا وجدتُ عندهم هذه المرّة

موقعهم الحيّ **محجوبٌ عن هذه البيئة** (بوّابة الخروج تمنع `worldmonitor.app`)،
فلا شيء هنا مستنتَجٌ من صورة: كلّه من وثيقة معمارهم نفسها، مقروءةً اليوم.
وعددهم تحرّك: ٥٣٦+ مضيفًا في القراءة السابقة، **٥٧٨+ الآن** — وهو بلغة §2a
عددُ *تكاملات*، وهو الرقم الوحيد لديهم القابل للمقارنة برقمنا.

الجدول الكامل في `docs/COMPETITORS.md`. وأصدق ثلاثة فروق ضدَّنا:

1. **تعلُّمٌ آليٌّ داخل المتصفّح** — تضمينات MiniLM عبر ONNX وبحثٌ دلاليّ في
   IndexedDB. بلا مفاتيح، وعلى عتادٍ لا ندفع ثمنه. هذا أحدُّ فرقٍ في المسح كلّه.
2. **مسارُ إقلاعٍ سريع** — `/api/bootstrap` بمهلتين (سريعة ٣ث وبطيئة ٥ث) وترطيبٌ
   مرّتين. عيبُنا المقيس هو ٦٠ ثانيةً في الإقلاع البارد.
3. **اختبارات انحدارٍ بصريّ.** كلّ عيبٍ تصميميٍّ في هذا المشروع حتى اليوم —
   ترويسةٌ منزاحةٌ ٢٠٨px، وكرةٌ تتوقّف عند ٥٧٤px، ورفٌّ يرسم فراغًا — **نجح في
   ٢٣٠٩ اختبارات** وبان في لقطةِ شاشةٍ واحدة. هذه ليست ملاحظةً عنهم، بل حكمٌ
   على اختباراتنا.

وما لا نأخذه: شفرتهم. AGPL‑3.0 تعني أنّ سطرًا واحدًا منها يفتح منتجنا كلّه.

## R276 — «اعمل أقوى تصميم وأدقّ وأكثر تطوّرًا» + مرجعٌ بصريّ (Z.E.R.O.)

نصّ الطلب، حرفيًّا:

> اكمل المطلوب واعمل اقوى تصميم وادق وتطوراا وسارسل لك صور يمكن تحتاجها
> لتوضيفها في ادواة قياس او شرح او رسم او غيره او حتى اذا كان مشروعنا يحتاج
> مثلها بالضبط او نوضفها في اماكن اخرى خاصة الذي يحلل ويراقب الاسواق او اي
> شيئ معين وهو ميزة مميزة لمشروعنا

مع صورةٍ وثلاثة مقاطع فيديو لواجهة **Z.E.R.O. — NEURAL SPACE**: رسمٌ ثلاثيّ
الأبعاد لشبكةٍ من العقد، عناقيدُ ملوّنة معنونة، خيوطٌ مضيئة بين العناقيد،
ولوحةُ حالةٍ يمنى بصفوفٍ حيّة، داخل إطارٍ بزوايا مقوّسة.

**ملاحظةٌ تقنيّة تُقال بصراحة:** لم أستطع استخراج إطارات الفيديوهات — نسخة
Chromium في هذه البيئة **بلا مفكّك ترميز H.264** (`canPlayType` تعيد فراغًا)،
وبناء ffmpeg المرفق مع Playwright مجرّدٌ منه أيضًا. فما بنيتُه مبنيٌّ على
الصورة الثابتة المرفقة وعلى وصف الطلب، لا على المقاطع.

**والقرار:** لا نستنسخ الشكل على بيانات وهمية. الصورة تُري *بنية* — عناقيد
وخيوط — وعندنا بنيةٌ حقيقيّةٌ من الطراز نفسه ولم نرسمها قطّ: **شبكة ارتباط
الأسواق**. المصدر بلا مفتاح: `sparkline_in_7d` من CoinGecko يعطي **١٦٨ سعرًا
ساعيًّا** لكلّ أصل في طلبٍ واحد — سلسلةٌ زمنيّةٌ حقيقيّةٌ متعدّدة المتغيّرات.

والطريقة ليست زينة: مصفوفة ارتباط على العوائد اللوغاريتميّة، ثمّ مسافة
Mantegna ‏`d = √(2(1−r))`، ثمّ **شجرة الامتداد الدنيا** (شجرة الأصول) وتعنقدٌ
هرميّ بالربط المتوسّط. هذه طريقةٌ منشورةٌ في اقتصاد الفيزياء منذ ١٩٩٩، وهي
تُنتج بالضبط الشكل الذي في الصورة — لأنّ الشكل في الصورة هو ما تبدو عليه بنيةُ
ارتباطٍ حقيقيّة.

**منفَّذ.** المقياس الحيّ: ١٠٠ أصل قُرئت، **٩٨ استُعملت**، ١٦٧ ملاحظة ساعيّة،
٩٧ رابطًا في شجرة الأصول، عشر مجموعات. وأشدّ زوجين وجدتهما الطريقة هما رمزا
الذهب المرمَّزان (r=0.989) وصندوقا الخزانة باليورو (r=0.990) — وهذا تحقّقٌ من
الطريقة لا زينةٌ لها.

وعطلان في التخطيط اكتُشفا بالقياس لا بالنظر: تماسكُ العنقود كان يتدرّج **خطّيًّا**
مع حجمه، فعنقودٌ من ٨١ عضوًا شدّ بواحدٍ وثمانين ضعفًا وانهار إلى عقدةٍ لا تُقرأ
(نسبة نصف القطر ٩٠٪ إلى الوسيط **٤٫٥٧**؛ بعد تخميدها بالجذر التربيعيّ **٢٫٣٧**)؛
والكاميرا كانت تؤطّر أبعد عقدة، فيتقلّص جسم السوق إلى خُمس الإطار لتبقى شاردتان
داخله — صارت تؤطّر المئين التسعين.

**وما يقوله عن نفسه على الشاشة:** عدد الملاحظات، طول النافذة، اسم كلّ خطوة
(Mantegna · MST · الربط المتوسّط)، وكلّ أصلٍ استُبعد **بالاسم وبالسبب وبالرقم**
الذي أوجبه. وأيضًا تركّزه: ٨١ من ٩٨ في مجموعةٍ واحدة — وهذه صفةُ سوق العملات
الرقميّة لا خطأٌ في التعنقد، ومن يرى كتلةً كبيرةً واحدة يستحقّ أن يُقال له أيّهما.

## R277 — لقطات «Pi Network Radar» للموقع القادم

نصّ الطلب، حرفيًّا:

> خاصة هذي الفيديوهات على الاغلب نحتاجهم بالضبط والصور لمووقع قريبا ساعمل عليه
> ارسلته لك فقط بمكن تاخذ او تستنتج منه افكار او تصميم او ملاحضة وتستغلها في
> مشروعنا

خمس لقطات للوحة قيادة Pi (نسختان: إنجليزيّة وعربيّة كاملة من اليمين). ليست
منافسًا نحلّله، بل **لغةُ تصميمٍ** يريدها المالك. وأربعة أشياء فيها ليست عندنا
وتستحقّ البناء، سُجّلت مهامًّا:

1. **شريط مؤشّرات علويّ** — ثماني بطاقات، كلٌّ بقيمة وفرق ومنحنى صغير.
2. **مبدّل طريقة العرض** — كرة ٣د / خريطة / شبكة / حرارة / تدفّق / خطّ زمن /
   رادار / رسم. عندنا اثنتان فقط، وقد صار عندنا **الشبكة** اليوم (R276).
3. **متحكّم كثافة العرض** — أدنى / متوازن / استخباراتيّ / أقصى. فكرةٌ مبتكرة
   فعلًا ولوحاتنا كثيفة، وهي أنسب لنا منها لهم.
4. **شريط حالةٍ سفليّ** — آخر تحديث، عدد المصادر، درجة الثقة، الشبكة. وهذا
   نستطيع أن نفعله **أصدق منهم**: عندنا `ok/empty/failed` لكلّ مصدر وتقدير
   Admiralty لكلّ بند، فبدل «Confidence: High» بلا سند نعرض العدد الذي وراءها.

وما لا نأخذه: الأرقام. كلّ رقمٍ في تلك اللقطات نموذجٌ مصمَّم (٤٧٬٨٩٦ عقدة،
$47.89) ولا يدخل مشروعنا رقمٌ لم يقسه مصدر.

## R278 — «الأسعار بلحظتها والأخبار بلحظتها، وزيادة المصادر، وتأكّد أنّه شغّال»

نصّ الطلب، حرفيًّا:

> اكمل عملك والدقة والاسعار بلحضتها والاخبار بلحضتها وخاصة زيادة المصادر
> والتقنيات والقياسات والمراقبة والادوات وان الذي نفعله هو مضبوط وشغال باقصى
> قوى ودقة

**الترتيب الذي اخترتُه، ولماذا:** «تأكّد أنّه شغّال» يسبق «زِد». إضافةُ مصادر
فوق مصادر معطّلة تُكبّر الرقم وتُصغّر المنتج. فأوّلًا قياسٌ حقيقيّ لما يعمل وما
لا يعمل، ثمّ الإصلاح، ثمّ الزيادة.

### أوّلًا: قياسُ ما يعمل — ٢٠٨ تغذيات، واحدةً واحدة

سألتُ كلّ ناشرٍ في الفهرس مباشرةً (`scripts/audit-feeds.ts`)، وهذه النتيجة
الكاملة لا خلاصةً مريحة:

```
٢٠٨ مفحوصة · ١٤٢ مقروءة · ٢٠ صامتةٌ فعلًا · ١ تنشر ما لا نستطيع قراءته · ٤٥ غير قابلة للوصول
```

و«غير قابلة للوصول» ليست حالةً واحدة، وهذا ما يهمّ:

```
١٦ → 403   ١٣ → 404   ٤ فشل اتّصال   ٤ انتهت المهلة
 ٢ → 500    ١ → 451    ١ → 410        ١ → 401   ١ JSON مبتور
```

**والقرار:** ٤٥ تغذيةً ميّتة تُصلَح قبل أن نضيف ٤٥ جديدة. إضافةُ مصادرَ فوق
مصادر معطّلة تُكبّر الرقم وتُصغّر المنتج، وهذا بالضبط ما يمنعه §2a.

وأُصلح اليوم ما يقبل الإصلاح صدقًا:

- **infobae** — الرابط القديم ٤٠٤ منذ فهرسته، وصفحة الناشر تعلن رابطًا آخر
  يجيب **200 بـ٨٦٩ كيلوبايت**. أُعيد توجيهه وخرج من الحجر.
- **skynewsarabia** — رابطه المُعلَن يجيب **403**: تحدٍّ آليّ، و§3 من الميثاق
  تمنع الالتفاف عليه. بقي في الحجر، وصُنّف `bot-blocked` بدل `moved` — لأنّ
  «انتقل» و«رفضني» ليسا الشيء نفسه ولا يُعالجان بالطريقة نفسها.

و**opensky_states** ينشر سجلّاتٍ لا يقرؤها مُحوِّلنا: خطأُ تعيينٍ عندنا لا
ساعةٌ هادئة عندهم، وهو مسجَّل للإصلاح.

### ثانيًا: الأسعار بلحظتها — الشريط الحيّ

**العطل المقيس:** كلّ سعرٍ في المنصّة كان يُحدَّث كلّ **١٢٠ ثانية**، وهذا رقمُنا
نحن لا إيقاعُ السوق.

**والحلّ:** مقبسٌ ويب مباشر إلى منصّةٍ عامّة بلا مفتاح، خلف واجهةٍ نملكها.
مقيسًا على الحقيقة: **Coinbase فتح في ٢٢٣ مللي ثانية وسلّم ستّ صفقات في ١٫١
ثانية**؛ Kraken فتح في ١٫٣ ثانية. و**Binance مستبعَدٌ عمدًا**: يجيب **451 —
ممنوعٌ قانونًا** من مخرج هذا النشر، ومصدرٌ يردّ ٤٥١ لا يُعاد طلبه من طريقٍ آخر.

والمتصفّح هو من يمسك المقبس، لا الخادم: سقف Netlify للاتّصال الممسوك ~٣٠ ثانية،
وشريطٌ يموت كلّ نصف دقيقة أسوأ من الاستطلاع.

**وثلاثة أعطالٍ وجدها القياس لا القراءة:**

1. `JSON.parse('null')` لا يرمي — يعيد `null`. فكان قارئا المنصّتين يفكّان
   مؤشّرًا فارغًا، وخطأٌ واحد داخل `onmessage` يُسقط الشريط لبقيّة الجلسة.
2. **إعادةُ المحاولة لم تكن تعمل أصلًا.** في متصفّحٍ حقيقيّ أمام منصّةٍ محجوبة:
   مقبسٌ واحد في ثلاثين ثانية، لا إعادة ولا تحوّلٌ إلى الثانية. القراءة لم تجدها؛
   استخرجتُ آلة الحالة إلى `lib/markets/tape-runner.ts` بمقبسٍ وساعةٍ يحقنهما
   المتصل، فوجدها اختبارٌ واحد. **والتحقّق بعد الإصلاح في المتصفّح: محاولتان،
   الثانية بعد أن انتهت مهلة الأولى فعلًا.**
3. **الشريط كان يكذب.** يقول «Opening the tape…» إلى الأبد بينما الاتّصال
   يُرفَض مرارًا. صار يقول: «Reconnecting to Coinbase — Socket error. واللوحة
   تحته مستطلَعة وتحمل أختامها الزمنيّة». وهو الفرق نفسه بين ضوءٍ أخضر وحقيقةٍ
   مقيسة.

وسياسة الأمان (CSP) تسمح بالمقبسين، واختبارٌ يمنع انحراف القائمتين — لأنّ أصلًا
ناقصًا منها يفشل في المتصفّح وحده، على الموقع المنشور وحده، وبصمت.

## R279 — خمسة طلبات: الرموز بدل الأرقام، الكوكب في الوسط، والجلسة لا تُفقد بالتحديث

نصّ الطلب، حرفيًّا:

> وايضا نضام الارقام ملونة على الخريطة والكوكب لم يعجبني وغير فعال وغير مفهوم
> ابدا نريد كل شيئ برمزه وحركاته وتفاعله ورمزه وتوضيحه و تجسيده كما يناسبه
> بالضبط وايضا تذكر تصميم صفحة الكوكب كلها لم ارى الجديدة وايضا اجعلها هي
> الصفحة الوسطى بين الصفحات وتكون ايقونتها اكبر قليلا او مميزة من بين الاربعة
> وعند فتح المشروع تفتح مباشرة عليها وايضا اريد عند زيادة او تحديث او تحميل
> فروع او اي شيى للمشروع فالمستخدم لا يعيد تسجيل حسابه ومعلوماته من الاول كل
> تحديث يجد اعادة التسجيل وغيره وايضا الاسعار والمعلومات لم ارها وكثير من
> الاشياء وايضا سرعة فتح التطبيق او الموقع

**والترتيب:** فقدانُ الجلسة عند كلّ تحديث أوّلًا — لأنّه عطلٌ يطرد المستخدمين،
والباقي تحسينات.

### منفَّذ من R279: الكوكب في الوسط، وهو ما يُفتح عليه

- **الترتيب:** `feed · markets · globe · intelligence · monitor` — مركزٌ حقيقيّ،
  لا تقريبيّ. وذلك تطلّب إخراج **الإعدادات** من الشريط السفليّ: خمس وجهاتٍ
  وشاشةُ إعدادات = ستّة أهدافٍ متزاحمة على هاتف ٣٦٠ بكسل، والإعدادات ليست وجهةً
  يقصدها أحد — بل مكانٌ يُذهب إليه مرّةً ويُغادَر. صارت في الترويسة، أعلى اليمين،
  حيث تعيش الحسابات في كلّ تطبيق، **وظاهرةً دائمًا** — مسجَّلًا دخوله أو لا، لأنّ
  اللغة فيها، ومن لم يسجّل أحوجُ إليها من غيره.
- **الزرّ الأوسط مميَّز:** قرصٌ ممتلئ يعلو الصفّ قليلًا وأيقونةٌ أكبر. لا زخرفة:
  الخانة الوسطى هي التي يبلغها الإبهام دون أن تتحرّك اليد، وشريطٌ من خمسة أهدافٍ
  متطابقة يقول إنّ كلّ الوجهات هي المقصد بالتساوي — وهذا غير صحيحٍ هنا، وهو
  اعتراض المالك بالضبط.
- **`/` صار الكوكب.** والتغذية احتفظت بمسارها الخاصّ: صفحةُ البدء دعوى عمّا
  يكونه المنتج، ودعوى هذا المنتج هي العالم لا قائمةُ منشوراتٍ عنه.

والتحقّق في متصفّحٍ حقيقيّ على ٣٩٠ بكسل: `/` تفتح على الكوكب، والشريط خمسة
أزرارٍ والكوكب في وسطها مرفوعًا.

## R280 — «صفحة الخريطة مثل worldmonitor.app/dashboard، مع لمستنا»

نصّ الطلب، حرفيًّا:

> وتذكر انا اريد تصميم وترتيب صفحة الخريطة تكون مثل موقع
> https://www.worldmonitor.app/dashboard?lat=0.0000&lon=-1.3773&zoom=1.00&view=africa&timeRange=7d&layers=conflicts%2Chotspots%2Csanctions%2Cweather%2Coutages%2Cnatural
> وكل ماهو موجود فيه والترتيب وكل العرض وغيرها فقط نضيف له لمستنا وبصمتنا
> والاشياء التي عندنا وايس موجودة فيه

**وأنفعُ ما في الرابط نفسه ليس الصفحة بل عنوانها.** انظر إليه:
...
هذه **حالةُ لوحةٍ كاملة داخل الرابط**: الكاميرا، والإقليم، والنافذة الزمنيّة،
ومجموعة الطبقات. ومعناها أنّ محلّلًا يستطيع أن يرسل لزميله *ما يراه بالضبط*.
وعندنا **لا شيء** من هذا: حالة الكرة كلّها في الذاكرة، فلا أحد يستطيع مشاركة
مشهد. وهذا أوّل ما يُبنى، لأنّه يجعل كلّ لوحةٍ بعده قابلةً للمشاركة.

### منفَّذ من R280 (الخطوة الأولى): المشهد في شريط العنوان

عندنا الآن ما في رابطهم، بمفرداتنا: `?view=map&layer=latency&region=all&window=24`.

وآليّتان لا تتداخلان: **التفضيلات** تجيب «ما الذي أريده عادةً» — تبقى للقارئ عبر
أجهزته وهي الافتراض حين لا يقول الرابط شيئًا؛ و**الرابط** يجيب «إلى أيّ شيءٍ
أشير الآن» — يغلب التفضيلات في تلك الزيارة وحدها.

وفرقٌ دقيق كلّفني تصميمًا ثانيًا: **شريط العنوان وزرّ المشاركة يريدان شيئين
مختلفين.** العنوان يبقى نظيفًا فيحذف ما لم يغيّره القارئ؛ أمّا رابط المشاركة
فيجب أن يكون **دقيقًا**: لو كان المُرسِل على طبقة الأحداث لأنّها الافتراض، فرابطٌ
يحذف `layer` يُري المستقبِل *طبقته هو*. فصارتا دالّتين، لكلٍّ صدقُها.

والمُحلِّل الذي يرسل رابطًا يرسل ما يراه — وهذا أنفع ما في تلك اللوحة كلّها.

**تحقّقٌ في متصفّحٍ حقيقيّ:** رابطٌ مشترك يفتح على الطبقة التي يسمّيها؛ ونقرةٌ
على شريحةٍ تُحدِّث العنوان؛ وزرّ المشاركة ينسخ الرابط الكامل.

**وما بقي من R280 كبيرٌ وأقوله صراحةً:** رفٌّ أيسر للطبقات، وشريطُ مؤشّراتٍ
علويّ، وشبكةُ لوحاتٍ سفليّة، ورموزٌ بدل الأرقام الملوّنة (R279 §1). هذه بناءٌ لا
ترقيع، ومسجّلةٌ مهامًّا.

### منفَّذ من R279 §1: رمزٌ لكلّ شيء، وحركةٌ تصفه

الاختصار المرفَق يشير إلى **تطبيقٍ مكتبيّ** لهم (`world-monitor.exe`) — يؤكّد
عميل Tauri الذي قرأتُه في وثيقة معمارهم. مسجَّلٌ في `docs/COMPETITORS.md`، ونحن
خلفهم في هذا العميل تحديدًا.

وأمّا الأرقام الملوّنة فقد زالت. **٢٥ فئة، لكلٍّ شكلُها وحركتُها وقراءتُها:**

| النوع | الشكل | الحركة | لماذا هذه الحركة |
|---|---|---|---|
| زلزال | حلقات | تتّسع للخارج | هذا ما تفعله جبهة الموجة |
| عاصفة | حلزون | يدور | الإعصار يدور |
| حريق | لهب | يرتعش ارتفاعًا | النار ليست ثابتة |
| فيضان | موجة | تتذبذب | الماء يسير |
| بركان | مخروط وعمود | العمود يعلو ويخفت | الرماد يصعد |
| وباء | نواةٌ وأقمار | تدور حولها | انتشارٌ من مصدر |
| نزاع | ضربتان متقاطعتان | تضربان على الإيقاع | أثرٌ لا انسياب |
| سيبراني | مسدَّس ومسح | يمسح من أعلى لأسفل | كنسٌ في نظام |
| طاقة | صاعقة | تومض | الكهرباء إمّا موجودة أو لا |
| نقل | سهم | يسير على محوره | حركةٌ على مسار |

**والقاعدة:** الحركة تصف *ما هو الشيء*، لا مدى إلحاحه — الإلحاح شدّةٌ وله حلقته
الخاصّة. ولا إيموجي ولا خطّ أيقونات: كلّ شكلٍ خطوطٌ متّجهة على لوحتنا، لأنّ
الإيموجي صورةٌ مختلفة على كلّ نظام تشغيل ولا تُقرأ عند عشرة بكسلات.

**والعدد لم يُحذف** — بل انتقل إلى شارةٍ صغيرة أعلى الشكل. هو معلومةٌ حقيقيّة،
لكنّه السؤال الثاني لا الأوّل.

**وصفّ الشرائح صار هو المفتاح:** كلّ شريحةٍ تحمل الشكل نفسه بالضبط، باللون نفسه،
يتحرّك الحركة نفسها. فالمفردات تُتعلَّم في الصفّ الذي يستعمله القارئ للتصفية، لا
في لوحةٍ عليه أن يبحث عنها.

**وثلاثة عيوبٍ وجدها الاختبار لا العين:** لوحةٌ تسجّل بدل أن ترسم كشفت أنّ الموجة
والسهم والمسح تقفز جميعًا إلى بدايتها في نهاية الدورة **وهي ظاهرة** — وميضٌ
مرئيّ. وأنّ اختباري الأوّل لـ«الحركة» كان ساذجًا: قارن طورين فقط، فحكم على
الصاعقة بالسكون لأنّها مضاءةٌ في كليهما.

## R281 — «عملت الدمج والنشر، أكمل بدقّة وابتكار»

أوّل ما فعلتُ: **تدقيق النشر الحيّ**، لأنّ الادّعاء بلا قياسٍ هو ما نتجنّبه.

**النتيجة الأولى — الصفحة الرئيسة سليمة:** أوّل طلبٍ بعد النشر تجاوز ٦٠ ثانية
(بردُ الحاويّة)، ثمّ ٠٫٨ و٠٫٤٥ و٠٫٢٢ ثانية. و`/` صفحةٌ **ثابتة** في البناء، فليس
هناك عملٌ عند الطلب — البرد هو البرد، وهو عيبُ السرعة المسجَّل أصلًا.

**والنتيجة الثانية — عطلٌ إنتاجيٌّ حقيقيّ في ثلاث واجهات، بسببٍ واحد:**

| الواجهة | ما ظهر |
|---|---|
| رأس المال الكلّي | **٢٫٦٨٩ تريليون دولار — حيّ** |
| المتحرّكون | **٠** |
| البورصات | **٠** |
| كوكبة الارتباط | **٠ أصل، مصدرٌ واحد فشل** |

والمسح نفسه قرأ **١٤٥ مصدرًا بنجاح** في اللحظة ذاتها — فلا شيء في النشر معطوب.

**السبب:** حدُّ المعدّل في المحرّك مفتاحُه **المصدر**، وأربعةٌ من مصادرنا تقرأ
`api.coingecko.com`. فكلٌّ منها يحمل مهلته الخاصّة، وكلّها تنطلق معًا في مسحٍ
واحد من عنوانٍ واحد. **كنّا ننفق ميزانيّة مزوّدٍ واحد أربع مرّات ونخسر ثلاثًا.**

**والحلّ:** ميزانيّةٌ **لكلّ مضيف**، يتشاركها كلّ مصدرٍ يقرؤه. ولا يكفي أن تكون
مهلةً أطول: المصادر الأربعة لا يعرف بعضها بعضًا، فالميزانيّة يجب أن تخصّ الشيء
المحدود فعلًا — المضيف.

وكان لا بدّ أن تتسلسل بصدق. الفحص القائم يقرأ آخر وقت، يحسب الانتظار، ينام، ثمّ
يكتب الوقت — وهذا تحت التزامن يجعل مستدعيَيْن يقرآن القيمة نفسها ويمضيان معًا.
كان سباقًا غير ضارّ حين لكلّ مفتاحٍ مستدعٍ واحد، وهو **المشكلة كلّها** حين
يتقاسم أربعةٌ مفتاحًا واحدًا. فصار لكلّ مضيفٍ سلسلةُ وعود: المستدعي يقف في
آخرها، ينتظر دوره، يأخذ فتحته، ثمّ يُفرج عمّن بعده.

**ويرفض بدل أن يصطفّ بلا حدّ:** من يقف الثانيَ عشر على مضيفٍ فاصلُه ثانيتان
سينتظر أربعًا وعشرين ثانية داخل طلبٍ يتخلّى عنه المنسّق عند الثامنة. والانتظارُ
لفتحةٍ لا نستطيع استعمالها ليس أدبًا. والتسعير **قبل** الانضمام لا بعده، وإلّا
انتقل سوءُ حظّ واحدٍ إلى كلّ من خلفه.

**وثانيًا: التقرير صار يقول لماذا.** كان يعيد «فشل مصدرٌ واحد» ولا شيء غير ذلك —
صورةٌ فارغة بلا سبب، وهي لا تُميَّز عن سوقٍ بلا بنية. والعدُّ بلا سببه هو الصمت
نفسه الذي يوجد هذا المشروع لإزالته.

**تصحيحٌ لنفسي، قبل أن يُصدَّق:** كتبتُ أنّ السبب واحدٌ خلف الصفوف الأربعة. هذا
أكثر ممّا يسنده الدليل. **داخل الطلب الواحد** — عدّةُ مصادر تقرأ المضيف نفسه في
مسحٍ واحد — التصادم حقيقيّ وهذا الطابور يزيله، وهو ما تثبته الاختبارات. أمّا
**بين الطلبات** فالميزانيّة تعيش في نطاق الوحدة، ونطاق الوحدة على هذا المضيف
غيرُ مضمون البقاء بين الاستدعاءات (الخاصّيّة نفسها التي جعلت «التمهّل» في البثّ
بلا أثرٍ هنا، وقد قِستُها في هذا المستودع من قبل). و`/api/chain` و
`/api/markets/constellation` طلبان منفصلان.

فالقول الصادق: **عطلٌ حقيقيّ أُصلح وأُثبت بالاختبار؛ وأمّا كونه *السبب* في فراغ
الإنتاج فلم يثبت بعد.** والذي سيحسمه هو النصف الآخر من التغيير نفسه: كلّ فشلٍ
صار يحمل كلام المزوّد، فالنشرة القادمة ستطبع سبب CoinGecko بدل صفرٍ صامت.

---

## R280a — لوحةٌ على هيئة `worldmonitor.app/dashboard`، وثلاثة تناقضاتٍ وجدتُها فيها

**الطلب (R280، حرفيًّا):** «اريد تصميم وترتيب صفحة الخريطة تكون مثل موقع
worldmonitor.app/dashboard وكل ماهو موجود فيه والترتيب وكل العرض وغيرها فقط نضيف
له لمستنا وبصمتنا والاشياء التي عندنا وليس موجودة فيه».

### ١. ما بنيتُه: شريطُ الأرقام، وسكّةُ الطبقات

**سكّةُ الطبقات.** لوحتهم تحمل طبقاتها في سكّةٍ يساريّة دائمة — ستّةُ مربّعات
اختيار وكلمة. الفكرة صحيحة ولم تكن عندنا: مفاتيحُ التصنيف كانت رقائقَ متناثرة في
بطاقةٍ **أسفل** اللوحة، فتغييرُ ما تَرسمه الخريطة يعني تمرير الشاشة بعيدًا عن
الخريطة، وفقدَ رؤية الشيء الذي تُغيّره، ثمّ العودة لترى ما حدث.

وسكّتُهم تقف عند مربّعٍ وكلمة. وثلاثةُ أشياء ناقصةٌ من ذلك يحتاجها المحلّل:

| | عندهم | عندنا |
|---|---|---|
| **كم يشكّل هذا الصنف من الصورة** | لا شيء | شريطُ نسبةٍ في كلّ سطر — فيُرى فورًا أنّ عالمًا يبدو مزدحمًا هو مصدرٌ واحدٌ مزدحم |
| **شكلُ العلامة** | مربّعُ اختيار | **الرمزُ نفسه** الذي ترسمه اللوحة، بلونه وحركته — فالسكّة هي المفتاح، يُتعلَّم حيث يعمل القارئ |
| **ما هو صامت** | ستّةٌ ثابتة | الفهرس كلّه، و**الأصناف التي لا تُبلّغ شيئًا مذكورة** — لأنّ صمت صنفٍ نتيجةٌ، وسكّةٌ تسرد ما وصل فقط تُعيد تعريف «لا زلازل مُبلَّغة» إلى «الزلازل ليست ممّا نتابع» |

وأضفتُ **«فقط»** بنقرةٍ واحدة في كلّ سطر: عزلُ صنفٍ بمربّعات الاختيار يكلّف
n−1 نقرة ذهابًا ومثلها عودة.

**شريطُ الأرقام.** ستّةُ أرقامٍ فوق الخريطة، محسوبةٌ في `lib/world/kpis.ts`
ومُختبَرة هناك (٣٤ اختبارًا). أربعةٌ منها لا يتطوّع بها لوحُ قياسٍ يريد أن يبدو
سليمًا:

- **التغذيات** تعدّ الرفض منفصلًا عن الفراغ. «أجاب بلا شيء» و«رفض أن يجيب»
  حقيقتان مختلفتان، وواحدةٌ منهما فقط مشكلتنا.
- **التطابق** يُعدّ بالأصول المستقلّة، فأربعون منفذًا تنقل برقيّةً واحدة = أصلٌ واحد.
- **المناطق العمياء** — نسبةُ ما لا نرى فيه أصلًا. الهادئُ وغيرُ المراقَب
  متطابقان على كلّ خريطةٍ أخرى، والفرق أهمّ ما يكون حيث التغطية أرقّ.
- **الحافّة الحيّة** — عمرُ أحدث *رصد*، لا عمرُ آخر جلب.

### ٢. وثلاثةُ تناقضاتٍ رأيتُها في اللوحة نفسها بعد بنائها

فتحتُها في متصفّحٍ حقيقيّ عند أربعة أحجام، فوجدتُ الصفحةَ تروي ثلاثَ رواياتٍ
متعارضة في وقتٍ واحد.

**(أ) «الحافّة الحيّة: الآن» فوق تغذياتٍ صفر.** الشريط يطبع **just now** بالأخضر
بينما اللوحة بجانبه تسرد العناصر العشرة نفسها بـ«قبل ٣ ساعات» و«قبل ٢١ ساعة»
و«٢٦ أغسطس». والسبب: `newestAt` كان يختزل على `e.at`، وهو `retrievedAt` —
**ساعتُنا نحن**، مختومةً على السجلّ لحظةَ وصوله. فكان الرقم عاجزًا حسابيًّا عن
أن يكون شيئًا غير اللحظة الحاضرة.

وأربعةُ أسطحٍ تقتبسه بوصفه «هل هذا حيّ؟»: ترويسةُ اللوح، البوّابات المثبّتة،
«الأحدث» في الموجز الدائم، والشريط. وكلّها كانت تجيب عن سؤالٍ لم يسأله أحد —
*هل أرسلنا طلبًا للتوّ؟* — بنبرة سؤالٍ يهمّ.

الإصلاح عند المنبع: القراءة من `observedAt`، وهو وقتُ الناشر نفسه وقابلٌ للعدم
عمدًا. ومعه `untimed` — كم حدثًا وصل بلا تاريخ — كي يعرف من يرى «—» السببَ.
**بعد الإصلاح: `newestAt` ١١:١٠ مقابل جلبٍ عند ١٤:١٧.** ثلاثُ ساعاتٍ وسبع دقائق،
وهي بالضبط ما كانت اللوحة تعرضه طوال الوقت.

**(ب) جزيرةُ العدم.** الصفحة تقول «٠ من ١٠ على الخريطة» وتحتها جملة: «كلّ تقرير
في هذا المسح وصل بلا موقع، فلا شيء يُرسم». واللوحة ترسم علامةً مكتوبٌ عليها
**١٠** في خليج غينيا.

السبب: فروعُ النقاط تقرأ `r.event.lat as number` من سجلّاتٍ يضمّها الترتيب بلا
إحداثيّات عمدًا (لأنّ **القائمة** يجب أن ترتّب قرارَ عقوباتٍ إلى جانب زلزال).
**والتحويلُ الصوريّ ليس تحويلًا:** `null as number` يبقى `null` وقت التنفيذ،
والحسابُ يقسره إلى `0`، فحطّ كلُّ حدثٍ بلا موقع عند **٠° شمالًا ٠° شرقًا**.

وهذا بعينه ما يقسم عليه رأسُ الوحدة نفسها: «الحدث الحقيقيّ بلا إحداثيّ يُعرَض
بجانب الخريطة، لا يُطرح صامتًا، ولا — **وهو أسوأ** — يُرسم في موضعٍ مُخمَّن».
وجزيرةُ العدم أسوأُ تخمينٍ متاح، لأنّها تبدو نتيجة.

**(ج) نقطةُ الكسر قِستُها ولم أخترها.** جعلتُ السكّة عموديّةً من `xl`، فقِستُ:
هذا التبويب ينفق ٢٦rem على سكّة السياق من `xl` أصلًا، فعلى حاسوبٍ ١٤٤٠ صارت
اللوحةُ **٥٣٠ بكسل** — ثلثُ عرضها ذهب إلى عمودِ زينةٍ ثالث. فنقلتُها إلى `2xl`
حيث تُبقي اللوحةَ عند ٨٩٨ بكسل. ودون ذلك تصير السكّةُ صفًّا أفقيًّا فوق اللوحة
مباشرة — ملاصقةً لها، لا خلفها أبدًا.

**ما قِسته بعد الإصلاح:** أربعةُ أحجام (٣٩٠ / ٨٢٠ / ١٤٤٠ / ١٩٢٠ / ٢٥٦٠)، لا
فيضان أفقيّ في أيٍّ منها، والشريط ٦ أعمدة على المكتب و٣ على اللوح و٢ على الهاتف.

**الاختبارات:** ١٨٤ ملفًّا، ٢٥٠٣ اختبارًا، كلّها ناجحة. والبناء نظيف.

---

## R275a — اختباراتٌ في متصفّحٍ حقيقيّ، لأنّ الخلل الذي يهمّ لا يسكن داخل وحدة

**السياق:** في جلسةٍ واحدة وجد متصفّحٌ حقيقيّ ثلاثةَ أعطالٍ لم يكن لأيٍّ من
**٢٥٠٣** اختباراتِ الوحدة أن يمسكها: جزيرةُ العدم، والحافّةُ الحيّة المشدودة إلى
ساعتنا، ولوحةٌ ضُغطت إلى ٥٣٠ بكسل. وثلاثتُها **علاقات** — بين البيانات والرسم،
بين لوحتين، بين مكوّنٍ والصفحة التي يهبط فيها. واختبارُ الوحدة لا يمسك علاقةً لا
يرى طرفيها معًا.

### ما بُني

`tests/browser/` + `vitest.browser.config.ts` + `npm run test:ui` (يبني، يخدم،
يختبر، يُطفئ). خارج المسح السريع عمدًا: يحتاج بناءَ إنتاجٍ وخادمًا، وثلاثون
ثانيةً قبل أوّل تأكيد. وجعلُ المسح الذي يُشغَّل دائمًا بطيئًا هو الطريقةُ التي
يتوقّف بها عن أن يُشغَّل.

**اثنا عشر تأكيدًا، كلٌّ منها اشتراه عطلٌ وقع:**

| التأكيد | العطل الذي اشتراه |
|---|---|
| اللوحة تُسلَّم علامةً واحدة لكلّ حدثٍ **مُوقَّع** | جزيرةُ العدم. واللوحةُ الرسميّة معتمةٌ على الاختبار — تُقاس كما هي سواء رسمت الصواب أو الخطأ أو لا شيء — فصار السطح يعلن مدخله في `data-plotted` |
| «لا شيء يُرسم» تعني صفرًا مرسومًا | الشكل المحدّد للعطل، منصوصًا وحده |
| الحافّة الحيّة ليست وقتَ الجلب | «الآن» فوق صورةٍ عمرها ثلاث ساعات |
| اللوحة تأخذ أغلب جزئها | سكّةٌ قاست جيّدًا وحدها وأخذت ثلثَ اللوحة في سياقها |
| لا يُنفَق نصفُ الشاشة قبل اللوحة | ٨١٪ من هاتفٍ قبل العالم الذي تُقاس به |
| لا فيضان أفقيّ × ٥ أحجام × ١٠ صفحات | ميثاق S10 — كان تقريرًا يقرؤه إنسان، وتقريرٌ لا يُقرأ يوم يتغيّر ليس ضمانًا |
| كلّ زرّ مساحةُ لمسه ٤٤ بكسل | ٢٧ زرًّا حقيقيًّا |
| الشريط والسكّة موجودان في كلّ عرض | كسرُ نقطةٍ يُسقط أحدهما على شاشةٍ واحدة غيرُ مرئيّ في اختبار وحدة |

### وثلاثةُ تصحيحاتٍ لنفسي أثناء البناء — الاختبارُ كان المخطئ، لا التطبيق

**(أ) `networkidle` لا يقع أبدًا على صفحاتنا.** الشريطُ الحيّ وشريطُ الأسعار
يُبقيان مقبسًا مفتوحًا عمدًا. فانتهت مهلةُ `/intelligence` عند ستّين ثانية،
**فأسقطت خمسةَ اختباراتِ فيضانٍ عبر خمسة أحجام** لسببٍ لا علاقة له بالتخطيط.
وأداةُ اختبارٍ تُبلّغ عن عطلٍ في التطبيق حين العطلُ فيها **أسوأ من لا أداة**:
تُعلّم القارئ ألّا يصدّق الأحمر.

**(ب) قِستُ الصندوق المرسوم بدل مساحة اللمس.** أبلغ الاختبار عن **٥١** إخفاقًا،
وكان مخطئًا في أكثرها: المستودع يحلّ المسألة أصلًا بطريقةٍ أفضل — أداةُ
`.touch-target` تُكبّر *مساحة الإصابة* إلى ٤٤ بكسل عبر `::after` وتترك الصندوق
صغيرًا. ورقاقةُ ٢٨ بكسل بمساحة لمسٍ ٤٤ **تصميمٌ صحيح**. فصار القياس من
`::after` نفسه — أي أنّ الاختبار يتحقّق أنّ الآليّة **تعمل**، لا أنّ أحدًا تذكّر
كتابة الكلمة.

وبقي **٢٧** إخفاقًا حقيقيًّا، أُصلحت كلّها. وثلاثةٌ منها — أزرارُ التقريب —
احتاجت حلًّا آخر: هي مكدّسةٌ بفاصل ٤ بكسل، فمساحةُ إصابةٍ وهميّة بـ٤٤ ستمتدّ
ثمانيةً خارج كلٍّ منها وتتداخل مع جارتها اثني عشر بكسلًا. القارئ يضغط واحدًا
فيحصل على الآخر ولا شيء على الشاشة يفسّر. فأُضيفت `.touch-size`: الصندوقُ نفسه
يكبر، وعلى المؤشّر الخشن فقط.

**(ج) شغّلتُ `prettier` على ملفٍ لا يستعمله المستودع.** لا إعداد ولا تبعيّة، فأخذ
`npx` الافتراضاتِ — علاماتُ اقتباسٍ مزدوجة وفواصلُ منقوطة — وأعاد كتابة **٧١٨
سطرًا** بأسلوبٍ ليس أسلوبَنا، فأسقط اختبارًا يبحث عن `from '@/lib/engine/catalog'`
بعلامةٍ مفردة. أعدتُ الملفَّ من `git` وأعدتُ تطبيق تعديليَّ الحقيقيَّين بيدي؛
الفرقُ الآن ١١٧ سطرًا كلُّها منسوبةٌ إلى تغييرٍ مقصود أو إلى إزاحةٍ يفرضها.

**النتيجة:** ١٨٤ ملفًّا و٢٥٠٣ اختبارًا سريعًا ناجحة، و١٢ اختبارَ متصفّحٍ ناجحة،
والبناء نظيف. واللوحة تبدأ الآن عند **٥٢٪** من الهاتف بدل ٨١٪.

---

## R275b — أوّلُ ضوء: خريطةٌ صادقةٌ بسرعة، ثمّ العالمُ كلّه

**السياق:** ميثاقُ R279 يطلب «سرعة فتح التطبيق او الموقع»، وقياسي على هاتفٍ
مُحاكى: الصفحة لا تُظهر شيئًا نحو **تسع ثوانٍ**.

### القياس أوّلًا — وتصحيحُ تخمينٍ كتبتُه قبله

كنتُ قد استندتُ إلى تعليقٍ في `world-events.ts` يقول إنّ مزوّدَ أخبارٍ بطيئًا
يجب ألّا يؤخّر طبقةَ الأحداث المقيسة. القياس قلبها:

| الطور | المصادر | الزمن |
|---|---|---|
| تسجيل الفهرس | — | ٢ms |
| `world_events` | ١٣٥ | **٢٤٩١ms** |
| `news` | ٣٩ | ٥٦٥ms |

فالطبقةُ المقيسة هي العمودُ الطويل، وفصلُ الأخبار ما كان ليشتري شيئًا. وما
تكلّفه الـ٢.٥ ثانية ليس بطءَ مزوّد: أبطأُ استجابةٍ مفردة **٣٣٥ms** والوسيط
**٢٦ms** — إنّها ١٣٥ طلبًا تتزاحم على DNS وTLS والتحليل.

### ما بُني

**`FIRST_LIGHT`** — أربعةَ عشرَ مرجعًا عالميًّا للأخطار، والقاعدةُ ليست «الأسرع»
(السرعةُ صفةُ شبكةٍ في يوم) بل **نطاقٌ عالميّ + إحداثيّ**. من الـ١٣٥ الباقية:
**٣٩ منها `meteoalarm_*`** — تحذيرُ طقسٍ لدولةٍ أوروبيّةٍ واحدة لكلٍّ منها، وأوّلُ
أربعَ عشرةَ علامةٍ على خريطة العالم لا يجوز أن تكون تسعًا وثلاثين طلبًا إلى
أوروبا؛ ومعظمُ الباقي وطنيٌّ أو بلا جغرافيا (`bls_us`, `arxiv_cs`, `boj_japan`).

| القياس | first-light | full |
|---|---|---|
| داخل العمليّة | **٣٦٥ / ٧٧٣ms** | ٢٩٢٣ / ١٩٩٩ms |
| عبر HTTP، خادمٌ بارد لكلٍّ | **١.٧٦s** | ٢.٤٣s |

الجدولان يختلفان لأنّ الطلبَ البارد يدفع نحو ١.٣s لترجمة المسار، ويدفعها
المسارُ السريع أيضًا. **فالادّعاء الصادق: أوّلُ رسمٍ أسرعُ بـ٢٨٪ على هذه الآلة،
ومسحٌ أسرعُ ٣–٥ أضعاف حين تكون العمليّةُ دافئة.** أمّا رقمُ الإنتاج فلا أدّعيه:
هنا تُرفض أغلبُ المصادر خلال أجزاء من الثانية، وهناك ١٧٤ مزوّدًا حقيقيًّا على
شبكةٍ حقيقيّة — وهي الحالةُ التي تنفع فيها هذه القائمةُ أكثر. **توقّعٌ مُعلّل، لا
قياس**، ويبقى موسومًا كذلك حتّى ينطق نشرٌ بغيره.

**والصدق في الواجهة:** `tier` يسافر على السلك، وشريطُ الأرقام يقرؤه ويكتب
«Feeds · first» مع «العدد يرتفع حين يصل المسحُ الكامل». تحميلٌ بمرحلتين يُخفي
مرحلته الأولى هو الكذبةُ نفسها التي أزلناها طبقةً أعلى.

### وعطلٌ في كودي الخاصّ، ادّعى تعليقُه عكسَ ما يفعل

كتبتُ في `bootstrap()` أنّ حارسَ الطلب-الواحد «مُتجاوَزٌ عمدًا هنا». **ولم
يتجاوزه شيء:** `loadWorld` يعيد الوعدَ الجاري لأيّ مستدعٍ ثانٍ، فطلبُ
first-light أعاد وعدَ المسح الكامل و**لم يُرسَل أصلًا**. صار الحارسُ مفتاحُه
الطبقة. وأُضيف حكمٌ ثانٍ: **الصورةُ الأصغر لا تحلّ محلّ الأكبر أبدًا** — المسحان
يتسابقان، وعلى ذاكرةٍ دافئة يسبق الكاملُ السريع، وإسقاطُ الأصغر فوقه ينزع
أحداثًا من خريطةٍ كانت تحملها.

### ثمّ: كلُّ فتح صفحةٍ كان يشعل مسحًا كاملًا لـ١٧٤ مزوّدًا

اكتُشف حين أشبعتْ مجموعةُ اختبارات المتصفّح الخادمَ. وهي مسألةُ ميثاق (§٣:
احترام حدود المعدّل) قبل أن تكون مسألةَ سرعة: سؤالُ هيئةِ أرصادٍ وطنيّةٍ ١٧٤ مرّة
لأنّ ١٧٤ شخصًا فتحوا صفحةً في الدقيقة نفسها هو بالضبط ما تمنعه تلك القاعدة.
فأُضيفت ذاكرةُ مسحٍ مدّتها **٣٠ ثانية** لكلّ طبقة، مع حمايةٍ من التدافع (عشرةُ
قرّاءٍ في الثانية نفسها يتقاسمون مسحًا واحدًا). و`generatedAt` **لا يُعاد ختمه**،
فالعمرُ الذي يراه القارئ هو عمرُ المسح لا عمرُ الإصابة.

### وثلاثةُ تخميناتٍ خاطئة عن مجموعة الاختبارات، حتّى قِست

فشل اختبارٌ واحد بعد الباقي. خمّنتُ **خادمًا مُشبعًا** (خطأ: عشرون طلبًا متزامنًا
ويتعافى في ٨ms)، ثمّ **متصفّحًا منهكًا** (خطأ: إعادةُ تدويره لم تُصلح شيئًا).
فأضفتُ تسجيلَ حالاتِ `4xx` على `/api/`، فنطق الدليل فورًا: **429 Too Many
Requests**. حدُّنا ٣٠ طلبًا في الدقيقة لكلّ عنوان، ومجموعتي كانت العميلَ المسيء.

**وما كشفه ذلك في المجموعة نفسها:** اختباراتُ الفيضان كانت **تنجح** على صفحاتٍ
رُفض كلُّ نداءِ بياناتٍ فيها — هندسةٌ خضراء فوق مستندٍ فارغ، وهي عينُ العطل الذي
يطارده هذا المشروع طبقةً أعلى.

والحلُّ ليس استثناءً لـ127.0.0.1 — المنصّاتُ التي تُنهي TLS أمام التطبيق تقدّم
عنوانَ الوسيط إليه، فـ«ثِق بالحلقة المحليّة» تعني «ثِق بالجميع» على النشر الذي
يحتاج الحدَّ أكثر. الحلُّ: **صفحةٌ واحدة لكلّ مسار، يُعاد تحجيمها عبر الأعراض
الخمسة** بدل خمسِ تحميلات. عشرُ تحميلاتٍ بدل خمسين، والتغطيةُ نفسها، وداخل الحدّ
بلا انتظار — وهو اختبارٌ أصدق أصلًا: المُختبَرُ هو CSS يستجيب لعرض، لا مسارُ
البيانات خمسَ مرّاتٍ إضافيّة.

وأخيرًا ضُبطت قاعدةُ «ما هو عطل»: **401/403 ليست عطلًا** — المجموعةُ تتصفّح دون
حساب، ومنعُ مجهولٍ من `/api/monitors` هو التحكّمُ بالوصول يعمل؛ و**503 ليست
عطلًا** — تعني «لا قاعدة بيانات هنا»، وهي الإجابةُ الصادقة. يبقى عطلًا: **429**
و**5xx** غيرُ المعلنة.

**ملاحظةٌ للمالك:** حدُّ ٣٠/دقيقة سخيٌّ لقارئٍ واحد وضيّقٌ خلف NAT مشترك — شركةٌ
أو مشغّلُ هاتفٍ يضع آلافًا خلف عنوان. لم أغيّره: تعديلُ حدٍّ أمنيّ قرارُك.

**النتيجة:** ١٨٦ ملفًّا و٢٥٣٣ اختبارًا سريعًا، و٨ اختباراتِ متصفّح، والبناء نظيف.

---

## R282 — «اكمل كل ماتقدر عليه وقلي مابقي ساكمله انا»

الطلب (حرفيًّا): «تمام اكمل كل ماتقدر عليه وقلي مابقي ساكمله انا».

### تصحيحٌ أوّلًا: عطلٌ ادّعيتُه ولم يكن موجودًا

حملتُ في تقاريري بندًا يقول إنّ `opensky_states` ينشر سجلّاتٍ لا يقرؤها محوّلُنا —
«خللُ ربطٍ مسجَّلٌ ولم يُصلَح». **فحصتُه فلم يكن هناك خلل:**

- السجلّ **مستبعَدٌ عمدًا ببوّابة الرخصة** (`agreement-required`): شروطُ OpenSky
  تشترط اتّفاقًا مسبقًا للاستعمال التجاريّ. تحقّقتُ: `in ACTIVE set: false`.
- والمحوّلُ المبرمَج `lib/engine/sources/geo.ts` يقرأ متّجهَ الحالة بفهارسه
  الصحيحة (٥ خط الطول، ٦ خط العرض، ٧ الارتفاع، ٩ السرعة) مطابقًا لتوثيق OpenSky.

فأسقطتُ البند بدل أن أخترع له إصلاحًا. **ادّعاءُ عطلٍ غير موجود ليس أقلَّ ضررًا من
إخفاء عطلٍ موجود** — كلاهما يجعل تقريري غيرَ قابلٍ للتصديق.

## R277c — شارةٌ تستحقّ ادّعاءها

**ما كان:** درعٌ خضراء في الترويسة مكتوبٌ عليها «سلبي · قانوني». **قيمةٌ ثابتة.**
تُرسم بالكلمات نفسها واللون نفسه سواء كان الحارسُ يفرض شيئًا أو حُذف بالكامل. وهي
أخطرُ ادّعاءٍ على الصفحة — قولٌ في القانون والأخلاق، لقارئٍ لا يملك وسيلةً أخرى
للتحقّق.

وهذا هو الشكلُ نفسه الذي قضيتُ الجلسةَ كلَّها أزيله طبقةً أدنى: مصدرٌ يعيد `[]`
حين يُرفض، ولوحٌ يقول «١٣ مصدرًا ناجحًا» فوق لوحةٍ فارغة، وحافّةٌ حيّةٌ مشدودةٌ
إلى ساعتنا. **الشارةُ المثبَّتة هي العطلُ نفسه في المكان الذي يهمّ أكثر.**

**القاعدة:** كلُّ فحصٍ هنا يجب أن **يفشل لو أُزيلت الحمايةُ التي يفحصها**. وهذا
يستبعد النوعَ المغري — قراءةَ ثابت، أو إعادةَ صياغة قاعدةٍ يفرضها الكود أصلًا —
ولذلك ثلاثةٌ من الخمسة **تُشغّل** الحارسَ بدل أن تتفحّصه:

| الفحص | ما يُثبته |
|---|---|
| كلُّ مصدرٍ للقراءة فقط | ١٧٤ مصدرًا مسجَّلًا، ولا واحدَ نشط |
| كلُّ مصدرٍ يسمّي مزوّديه | لا مصدرَ يبلغ مضيفًا لم يعلنه |
| قائمةُ السماح ترفض المجهول | **سُئلت عن مضيفٍ لا يعلنه أحد فرفضته** |
| لا شيء يُرسَل يغيّر حالة | **جُرِّبت بـDELETE فرفضها الحارس** |
| الرُّخص مُنفَّذة | **٨٢ من ٢٤٧ مصدرًا محجوزةٌ برخصتها** |

و«غير معروف» ليست «قانونيّ»: أوّلُ رسمٍ هو الدرعُ المحايدة لا الخضراء. شارةٌ
تفترض الامتثالَ بينما تنتظر هي الشارةُ المثبَّتةُ نفسها مع تأخيرٍ أمامها.

### وعطلٌ ارتكبتُه داخل الوحدة التي تزيل هذا العطل

كتبتُ فحصَ الطريقة بـ`try/catch` متزامن حول `createFetch` — وهي دالّةٌ **غير
متزامنة**، فهي **ترفض** ولا **ترمي**. فما كان `catch` ليمسك شيئًا أبدًا، وكانت
الشارةُ ستُبلّغ عن حارسٍ معطوبٍ في كلّ رسم: **شارةُ امتثالٍ عالقةٌ على الأحمر
لسببٍ في الفاحص نفسه.** وهو بالضبط ما توجد هذه الوحدةُ لإزالته — ادّعاءٌ لا يفعل
الكودُ ما يقوله تعليقُه — ارتكبتُه وأنا أزيله. الإصلاحُ كلُّه `await`.

**والمستودعُ أمسك سهوًا آخر لي:** اختبارُ `api-catalog` رفض المسار الجديد لأنّ كلّ
مسارٍ يجب أن يُصنَّف عامًّا أو داخليًّا. صنّفتُه **عامًّا**: نشرُ الدليل خلف ادّعاءٍ
عامٍّ هو الغرضُ كلُّه، وادّعاءُ امتثالٍ لا يستطيع القارئُ فحصَه هو ادّعاءٌ يُطلب
منه أن يصدّقه.

**النتيجة:** ١٨٧ ملفًّا و٢٥٤٣ اختبارًا، و٨ اختباراتِ متصفّح، والبناء نظيف.

## R277a — مستوى الكثافة: أدنى / متوازن / استخباراتي / أقصى

**محورٌ واحد لا محوران.** الصفحةُ تحمل `panelSize` أصلًا (مضغوط/عادي/عريض) وهو
يتحكّم بتفصيل **الصفّ**. والبناءُ الواضح كان إضافةَ مقبضٍ ثانٍ بجانبه — وهذا خطأ:
مقبضان مستقلّان يعطيان القارئ تسعَ توليفات يريد منها ثلاثًا، ويتركان المنتَج
عاجزًا عن قول ما تعنيه «الكثافة» لأنّها صارت تعني شيئين. **فالكثافةُ هي المحور،
وهي التي تُحرّك حجمَ الصفّ.**

**وكلُّ مستوًى يعني شيئًا قابلًا للفحص** — لا «أكثر» فحسب. كلٌّ يسمّي **مهمّة**
القارئ لا حجمَه:

| المستوى | المهمّة | ما يفتحه | مقيسًا في المتصفّح |
|---|---|---|---|
| أدنى | هل يحدث شيء؟ | اللوحةُ والأرقام فقط | **٠ أقسام** |
| متوازن | ماذا يحدث؟ | القائمةُ المرتّبة وغيرُ القابل للتوقيع | **٢** |
| استخباراتي | هل أصدّق؟ | الدمج والمناطق العمياء وسلامةُ المصادر | **٤** |
| أقصى | أرِني كلَّ شيء | كلُّ قسم + صفوفٌ عريضة | **٤** + الأقسامُ التي لم تُخلق بعد |

و«أقصى» يفتح **ما لا وجود له بعد**: مستوًى يعني «كلَّ شيء» يجب ألّا يعني سرًّا
«كلَّ ما عرفتُه في أغسطس». قسمٌ يُضاف لاحقًا ويظهر مغلقًا عند «أقصى» هو الإغفالُ
الصامت نفسه الذي يطارده هذا المستودع.

### وعطلٌ ارتكبتُه ثمّ أصلحتُه قبل أن يُدفع

جعلتُ في المحاولة الأولى **مصدرَين للحقيقة**: مجموعةَ الطيّ المحفوظة + الكثافة،
مع محاولةِ التوفيق بينهما. النتيجة: نقرةٌ على قسمٍ أغلقته الكثافةُ تقلب قيمةً
مخزّنةً **لا يراها القارئ**، فلا يحدث شيءٌ مرئيّ. **ونقرةٌ لا تفعل شيئًا هي عطلٌ
يلوم القارئُ نفسَه عليه.** فصار التجاوزُ خريطةً واحدةً بسيطة، و`collapsedAt`
يجيب عن كلّ ما لم يلمسه القارئ.

**ولماذا لا يُحفظ التجاوز والكثافةُ تُحفظ؟** حفظُهما معًا مصدران مخزّنان لسؤالٍ
واحد، ويتناقضان لحظةَ تغيير الكثافة على جهازٍ آخر: التخطيطُ يقول «أدنى» وثلاثةُ
أقسامٍ مفتوحة بلا تفسيرٍ على الشاشة. **المستوى** هو الاختيارُ الدائم، و**السهم**
هو اللحظيّ. واختيارُ مستوًى يمسح التجاوزات لأنّ اختيار تخطيطٍ إعادةُ ضبطٍ مقصودة.

### واختبارانِ لي كانا مخطئَين عن تصميمي

طالبتُ بأنّ كلَّ مستوًى **أكثفُ تمامًا** ممّا تحته. وعبر الأقسام الخمسة الموجودة
اليوم، «استخباراتي» و«أقصى» يفتحان الخمسةَ نفسَها؛ و«أقصى» أكثفُ بطريقتين لا
يراهما العدّ: يفتح ما لم يوجد بعد، ويوسّع كلَّ صفّ. فأكّدتُ الاثنتين منفصلتين بدل
أن أحشر قسمًا سادسًا مصطنعًا في الجدول لإرضاء عدّاد.

وحقلُ `question` طالبتُ أن ينتهي بـ«؟» — وثلاثةٌ من أربعة كذلك، و«أرِني كلَّ شيء»
**طلبٌ لا سؤال**. فوسّعتُ اسمَ الحقل إلى `job` بدل أن أُعيد صياغة المستوى الوحيد
الذي لا يناسب قاعدةً اخترعتُها للثلاثة الأخرى.

**النتيجة:** ١٨٨ ملفًّا و٢٥٦٠ اختبارًا، و٨ اختباراتِ متصفّح، والبناء نظيف.

## R283 — تدقيقُ النشر الحيّ (أوّل قياسٍ حقيقيّ للإنتاج)

الرابط: `melodious-tiramisu-8edae7.netlify.app`. وهذا **أوّلُ قياسٍ للإنتاج** في
هذه الجلسة كلّها — كلُّ ما قبله كان صندوقًا تُرفض فيه أغلبُ المصادر في أجزاء من
الثانية.

### أوّلًا: تصحيحُ ادّعاءٍ لي، خلال دقيقتين من إطلاقه

قلتُ إنّ `/api/world` الكامل **فشل** (`http 000` بعد ١١.٣ ثانية). ثمّ أعدتُ
القياس مرّتين: **200 في ٣.٢٨s باردًا و١.٠٩s دافئًا**. كان عطلًا عابرًا في اتّصالي
لا في التطبيق. **المسحُ الكامل يعمل.**

### ما هو حيّ وما ليس

`/api/posture` يُعيد صفحةَ ٤٠٤ — فآخرُ دفعتَين (`dad3c8d`, `44c20b7`) **غيرُ
منشورتَين**. أمّا `?tier=first-light` فيعمل: **١.٤٦ ثانية** ببياناتٍ حقيقيّة.

### القياس

| | |
|---|---|
| المسحُ الكامل | ٣.٢٨s بارد · ١.٠٩s دافئ · **٦ ميغابايت** |
| الأحداث | ٢٢٢ موقّعة · ٣٩٢٩ بلا موقع · ٤١٥١ إجمالًا |
| المصادر | ٧ ناجحة · ٣٠ فارغة · **١٣٦ من الذاكرة** · ١ فاشلة |
| الأصناف | ٢٢ تُبلّغ |

و«١٣٦ من الذاكرة» **سلوكٌ صحيح** لا عطل: الحدُّ الأدنى لفاصل كلّ ناشر لم ينقضِ،
فيُعاد آخرُ جوابٍ أعطاه. هذا ما يجعل السبعةَ الطازجةَ سبعةً.

### عطلٌ حقيقيّ: ساعةُ ناشرٍ خاطئة تُثبّت اللوحةَ كلَّها

**`newestAt` كان ٤٤.٥ دقيقة *بعد* المسح الذي أنتجه.** خمسةَ عشرَ حدثًا بتوقيتٍ
في المستقبل، كلُّها من منفذٍ نيجيريّ واحد، موزّعةً بانتظامٍ أمام اللحظة — بصمةُ
ناشرٍ يختم توقيته المحلّيّ على أنّه UTC، لا خمسةَ عشرَ شيئًا لم يحدث بعد.

**ولماذا يهمّ أكثر من خمسةَ عشرَ صفًّا خاطئًا:** `ageWords` يقصّ العمرَ السالب
بـ`Math.max(0, …)`، فالتوقيتُ المستقبليّ يُرسم **«الآن»** — و`newestObservation`
يأخذ الأحدث من كلّ شيء، فساعةُ منفذٍ واحدٍ خاطئة **ثبّتت الحافّةَ الحيّة للوحة
كلِّها على «الآن» إلى الأبد**.

وهذا هو العطلُ الذي أصلحتُه صباحَ اليوم نفسِه، عائدًا **من بابٍ آخر**: كانت
ساعتُنا تُعرض كطزاجة العالم، وصارت ساعةُ ناشرٍ تفعل الشيء نفسه. والدرسُ واحد:
**رقمٌ لا يستطيع التعبير عن القِدَم سيُبلّغ عن الصحّة إلى الأبد.**

**الإصلاح: رفضٌ لا قصّ.** قصُّ التوقيت المستقبليّ إلى «الآن» يُخفيه ويُنتج
«الآن» الكاذبةَ نفسَها. حدثٌ لا نستطيع تأريخَه بصدقٍ هو حدثٌ بلا وقت — وللنموذج
كلمةٌ لذلك أصلًا: يصير `untimed`، و`summary.untimed` يعدّه حيث يراه القارئ.
والسماحيّة **خمسُ دقائق** لا صفر، لأنّ الساعات تختلف بثوانٍ.

### ReliefWeb: ٤١٠ Gone — وهو إجراءٌ عليك أنت لا إصلاحٌ في الكود

المصدرُ الفاشلُ الوحيد في مسحِ ١٧٤ مصدرًا. فحصتُه مباشرة:

- **v1 → 410 Gone.** الواجهةُ التي نناديها **أُلغيت**.
- **v2 → 403**، وبكلماتهم هم في جسم الجواب: *«You are not using an approved
  appname. Kindly request an appname from ReliefWeb.»*

و**هذا الـ403 منهم لا من وسيطنا**: وصل كخطأ JSON خاصٍّ بهم بينما طلبُ ضبطٍ إلى
USGS من الطرفيّة نفسها أعاد 200.

فالمحوّلُ ليس خاطئًا ولم يُحذف؛ الواجهةُ تحته أُلغيت وخليفتُها تحتاج **اسمًا
مسجَّلًا لدى UN OCHA**. أُخرج المصدرُ من التسجيل وبقي صفُّه في الفهرس معطَّلًا —
وهي قاعدةُ المشروع لمسارٍ يحتاج بيانَ اعتماد: **تبقى الفجوةُ مرئيّةً ومسمّاة.**

**وسهوٌ أمسكتُه في نفسي:** أطفأتُ `enabled: false` على صفّ الفهرس أوّلًا — و
`registerNewsGateway` يسجّل مصفوفةَ `newsGatewaySources` **لا صفوفَ الفهرس**. فكان
عَلَمًا لا يغيّر شيئًا: **الشكلُ نفسه من الخداع الذي أزلتُه طوال الجلسة، أرتكبه
وأنا أوثّق انقطاعًا.** أخرجتُه من المصفوفة الحقيقيّة.

### فيرسل: لا يوجد مشروع

حسابُ `algb20's projects` (خطّة hobby) فيه **صفرُ مشاريع**. فلا نشرَ على فيرسل،
والتطبيقُ الحيُّ هو Netlify وحده. و`vercel.json` في المستودع يعلن جداولَ cron
لا يقرؤها أحد — ولهذا يوجد `netlify/functions/scheduled-jobs.mts`.

### وملاحظةٌ للمالك: ٦ ميغابايت

جوابُ `/api/world` **٦٬٠٥٠٬٥٨٢ بايت**. على هاتفٍ بحزمة بيانات هذا ثقيلٌ جدًّا،
و«أوّل ضوء» يخفّفه للرسم الأوّل فقط. تقليصُه تغييرٌ في عقد السلك (٣٩٢٩ حدثًا بلا
موقع تُشحن كاملةً) — لم أفعله لأنّه قرارُ منتج، وأقترحه.

| R268 | `perf` | `done` | **اصلح خطا وسرعة فتح التطبيق** — fix the error, and the speed of opening the app. |

**R268 — عطلٌ واحد قِيس، وحِملُ فتحٍ نصفُه لم يكن لازمًا.**

### العطل: ثلاثةُ مسارات لا تنتهي من التحميل أبدًا

`scripts/walkthrough.ts` كان يسجّل `—` بدل رمزِ الحالة في `/intelligence`
و`/monitor` و`/account`: المسارُ لا يبلغ سكونَ الشبكة خلال ٤٥ ثانية. وبعزلِ
طلبِ `sdk.minepi.com` (لا يصله هذا الصندوق) ظهر السببُ نظيفًا:

| المسار | قبل | بعد |
|---|---|---|
| `/globe` | ٢٬٧٩٠ مللي | **٩٩٧ مللي** |
| `/monitor` | **لا يسكن أبدًا** | **٧٢٩ مللي** |
| `/intelligence` | **لا يسكن أبدًا** | **٧٤٥ مللي** |
| `/account` | **لا يسكن أبدًا** | **٧٥٠ مللي** |

السبب: `fetch` يعيد استجابةً جسمُها تيّار، والمتصفّحُ يُبقي الاتصال مفتوحًا حتى
يُقرأ ذلك التيّار أو يُلغى. وعشرُ نداءات في الواجهة كانت تقرّر من رمزِ الحالة
وحدَه ثمّ تنصرف — `if (res.status === 401) return` — فيبقى المقبسُ محجوزًا طولَ
عمر الصفحة. ولأنّ الاستجابة **وصلت** والشاشةُ **رُسمت** صحيحةً، لا شيء في
المتصفّح يقول إنّ شيئًا تعطّل: حدثُ `load` وحدَه لا يقع أبدًا، فيبقى المؤشّرُ
يدور والقياساتُ المعتمِدةُ عليه تنتظر إلى الأبد. `lib/http/discard.ts` يُطلقُ
الجسمَ صراحةً، و`discard.test.ts` يمسح كلَّ ملفّات الواجهة فلا تعود الصيغةُ
نفسُها تُكتب بحسن نيّة مرّةً أخرى.

### السرعة: كلُّ مسارٍ كان يُنزّل المنتجَ كلَّه

كلُّ لوحةٍ كانت استيرادًا ثابتًا في `app/page.tsx`، فوضع webpack المنتجَ كلَّه في
حزمةٍ واحدة نزّلها كلُّ مسار. مقيسًا في متصفّحٍ بحجم هاتفٍ على بناء الإنتاج:

| المسار | قبل (مفكوك) | بعد | على السلك: قبل → بعد |
|---|---|---|---|
| `/` (الخريطة) | ١٬٠٣٢ ك.ب | **٧٢٥ ك.ب** | ٣٠٨ → **٢٣٩ ك.ب** |
| `/intelligence` | ١٬٠٣٢ | **٧٢٧** | ٣٠٨ → **٢٣٤** |
| `/monitor` | ١٬٠٣٢ | **٥٤٤** | ٣٠٨ → **١٧٩** |
| `/markets` | ١٬٠٣٢ | **٥٦٢** | ٣٠٨ → **١٨٣** |
| `/account` | ١٬٠٣٢ | **٥٧٦** | ٣٠٨ → **١٨٩** |

و«First Load JS» الذي يطبعه البناء لـ`/` و`/[tab]`: **٣٠٣ ك.ب ← ١٥١ ك.ب**.

و`ssr` يبقى مُشغّلًا لكلّ لوحة عمدًا: كلُّ تبويبٍ مُسبَقُ التوليد على عنوانه، فيرى
الزاحفُ المنتجَ ويُرسم الرابطُ العميق من أوّل إطار، وNext يُحمّل مسبقًا الحزمةَ
التي استعملها ذلك التوليدُ وحدَها. `app/shell-code-splitting.test.ts` يحرس القاعدةَ
كلَّها — بما فيها أنّ لوحةً لا تُطفئ `ssr`.

### وحمولةٌ لم يكن لها أن تصل المتصفّح أصلًا

`originOf` أربعةُ أسطر تجيب «مَن الناشرُ خلف هذا المفتاح»، لكنّها كانت تقرأ
`CATALOG`، فسحبَ استيرادُها في مكوّنَي واجهة **١٩٢ ك.ب** من `lib/engine/catalog/feeds/`
إلى أوّلِ حزمةٍ في الصفحةِ الافتراضية: كلُّ رابط، وكلُّ حدٍّ للمعدّل، وكلُّ تقدير
أدميرالتي نملكه، مقروءًا في مصدر الصفحة. والخريطةُ نفسُها ١٨٧ مدخلًا — **٧٫٣ ك.ب**
— وهي ما يُشحن الآن، مولّدةً من الكتالوج بـ`scripts/build-origin-index.ts`
ومُثبّتةً مساواتُها له في `origins.test.ts`، فلا تنحرف نسخةٌ ثانيةٌ من مجموعات
الاستقلال عن المجموعات التي تعدّها درجةُ الثقة (§٢أ).

### ما لم يُغلق

R160 («كلُّ مسارٍ ~١٣٫٤ ثانية حتى سكون الشبكة») يبقى مفتوحًا رغم ما سبق: الزمنُ
المتبقّي في `walkthrough` هو طلبُ `sdk.minepi.com` وحدَه — يُحمَّل على كلّ سطحٍ
عبر `schedulePiProbe` ولا يصله هذا الصندوق فيتعلّق حتى ينقطع. يحتاج قياسًا على
شبكةٍ حقيقية قبل الحكم عليه، ولذلك لم يُشطَب.

## R284 — «عملتُ الدمج وأضفتُ المفتاحين ونشرت، بقيت خطوتان»

> **الطلب حرفيًّا:** «لقد عملت الدمج واضفت المفتاحين و عملت النشر متبقي خطوتين»

فذهبتُ أقيس النشرَ الجديد بدل أن أصدّقه. وما وجدتُه أنّ النشرَ **لم يخرج**،
وأنّ الصفحةَ التي وظيفتُها كلُّها أن تقول «ما العطل» كانت تكتم الجواب وهي
تحملُه في يدها.

### أوّلًا: البناءُ الحيّ متأخّرٌ باثنتي عشرة دفعة

`/api/health` يصرّح ببنائه: `51fb55a`، فرعُ `main`، بُني **٢٧ أغسطس ٢٠:٠٩**.
ورأسُ `main` اليوم `168b63f` — أي **١٢ دفعةً بعده**، ومنها دمجُ PR #66 الذي تمّ
الساعة **٠١:٤٥** من اليوم التالي، أي **بعد أربع ساعاتٍ ونصف من آخر بناء**.

ولهذا `/api/posture` يردّ **404**: الشارةُ التي تتحقّق من ضماناتنا ليست في
البناء الجاري أصلًا. فالدمجُ تمّ، والنشرُ لم يلتقطه.

**والمفتاحُ الأوّل وصل:** `pi_api_key = ok`. أمّا الثاني فلا — وهذا بابُ ما يلي.

### ثانيًا: `MAIL_PROVIDER` متغيّرٌ يُقرأ ثمّ يُهمَل في صمت

`createMailProvider` كان يقرأ `MAIL_PROVIDER`، يخفضه إلى الأحرف الصغيرة،
يقارنه بقيمتين (`disabled` و`log`)، **ويرمي كلَّ ما عداه**. فمن كتب
`MAIL_PROVIDER=resend` — وهو أوّلُ ما يخطر لمشغّل — ضبط متغيّرًا لا يفعل شيئًا.
ثمّ إنّ فحصَ الصحّة ينصحه: «اضبط مزوّدَ بريد» — وهو ينظر إلى متغيّرٍ اسمه
`MAIL_PROVIDER` قد ضبطه فعلًا. لا مخرجَ من هذه الحلقة بقراءة النصيحة، لأنّ
النصيحةَ تجيب عن سؤالٍ غير المسؤول.

والعلاجُ الدائم ليس جملةً أفضل في ثلاثة ملفّات، بل **قرارٌ واحدٌ وثلاثةُ قرّاء**:
`lib/mail/config.ts` — `planMail(env)` — هو الآن القارئُ الوحيد لهذه المتغيّرات،
ومصنعُ المزوّد وفحصُ الصحّة وطريقُ `/api/mail/test` يبلّغون ما قرّره. و
`MAIL_PROVIDER=resend` صار **يختار** Resend؛ فإن غاب مفتاحُه رفض **باسم Resend**
ولم يتسلّل إلى مفتاحٍ آخر ولا إلى SMTP: «الإجبار» يجب أن يعني الإجبار، وإلّا فمن
سمّى مزوّدًا وأتاه البريدُ من غيره قد خدعه المفتاحُ الذي استعمله ليكون صريحًا.
وقيمةٌ لا نعرفها — `sendgrid` — تُهمَل ولكن **لا في صمت**: تُسمّى وتُعرَض القيمُ
المقبولة.

### ثالثًا: مسبارُ الصحّة كان يشهد لنفسه بما ينفيه

`lib/auth/session` يرفض أيَّ سرٍّ أقصر من ١٦ محرفًا. وفحصُ `session_secret` كان
يسأل عن **الوجود** فقط. فـ`SESSION_SECRET=lambda` يعطي «configured» خضراء
وتُرمى استثناءً عند كلّ تسجيل دخول: **المسبارُ يؤكّد الشيءَ الذي وُجد لينفيه.**
صار الطرفان يسألان `isUsableSessionSecret` نفسها، فلا يفترقان مرّةً أخرى.

### رابعًا — وهو الأسوأ: `/setup` يرسل القارئ إلى سجلٍّ لا يملكه

`/api/health` يردّ **503** حين يغيب إعدادٌ مطلوب — رفضٌ صحيحٌ من خادمٍ سليم.
و`diagnose` كان يقرأ الـ5xx فيكتب «شيءٌ بالداخل يفشل» + «اقرأ سجلَّ الخادم»،
**ثمّ يتوقّف** — فلا يُعرض فحصُ الشبكة ولا البوّابات ولا قاعدةُ البيانات، وكلُّها
تعمل. صفحةٌ حمراءُ ببطاقةٍ واحدة، بينما اسمُ الإعداد الغائب في نفس الـJSON التي
جلبتها الصفحةُ قبل سطرين. وعلى مضيفٍ عديم الخادم، «اقرأ سجلَّ الخادم» نصيحةٌ لا
يقدر عليها أكثرُ الملّاك.

وهذا **عينُ الخلل الذي كُتبت هذه الوحدةُ لتمنعه، ترتكبه الوحدةُ نفسها.**

فالآن: تقريرٌ يسمّي فحوصَه الفاشلة ليس عطلًا في الخادم — يُقال الاسمُ ويُكمَل
باقي الصفحة. وأُضيف فحصان يظهران للقارئ لأوّل مرّة: **الحسابات والدخول**
(`SESSION_SECRET` باسمه، مع `openssl rand -base64 32`) و**بريدُ التسجيل
والاسترجاع**. كلاهما كان يسقط بلا أثر: الخريطةُ والبوّاباتُ والأخبارُ تعمل بدونهما،
فالعرَضُ الوحيدُ «التسجيلُ معطّل» بلا ما يُشار إليه.

### ما قِيس، لا ما رُجّي

شغّلتُ خادمًا حقيقيًّا بـ`SESSION_SECRET=short` و`MAIL_PROVIDER=resend` بلا مفتاح:

- `session_secret = degraded` — «set but only 5 characters — the signer requires 16 or more».
- `mail = off` — «MAIL_PROVIDER=resend is set, but RESEND_API_KEY is not… MAIL_PROVIDER only chooses which provider to use; it is not itself a credential.»

وقبل التغيير كانت البيئةُ نفسها تقول `session_secret = ok`، ونصيحةَ بريدٍ لا تذكر
`MAIL_PROVIDER` بحرف.

واختباراتُ الانحدار مُتحقَّقٌ منها بالنقض: بإعادة الشرط القديم في `diagnose`
تسقط **٥** منها.

## R285 — «زيبٌ كاملٌ، ودراسةٌ شاملةٌ للمشروع بكلّ التفرّعات»

> **الطلب حرفيًّا:** «اعطني ملف زيب كامل للمشروع / واعطني ملف فيه كل شيء عن المشروع
> بكل التفاصيل وكل ماعملناه وماتبقى والهدف اريد ملف شامل للمشروع الموقع والتطبيق
> وكل مايربطه لا تنسى اي تفاصيل بكل التفرعات / احتاج دراسة شاملة للمشروع»

سبقه طلبان: زيبٌ للرفع على «أب ستيديو»، ثمّ **«الملف أكبر من ١ ميغا لم يقبل»**.

### أوّلًا: المستودع فيه مُحزِّمٌ لهذا بالضبط، وكان معطَّلًا

ارتجلتُ ثلاثةَ أرشيفاتٍ يدويّةً قبل أن أجد `scripts/package.mjs` — مُحزِّمَ إصدارٍ
مكتوبًا لهذه الحالة نفسها، وفيه **فحصُ أسرارٍ يُفشِل الحزمة** إن تسلّل مفتاح. كان
عليّ أن أبحث قبل أن أرتجل.

وحين شغّلتُه: `npm run package` نجح (**٣٬٧٥٢ ك.ب**)، و`npm run package:studio`
**فشل** — **٢٬٨٨٢ ك.ب** مقابل سقفِ **١٬٠٢٤**.

**السبب:** ثلاثةُ ملفّاتٍ لا تُحمَّل في أيّ صفحة كانت **١٫٥٦ م.ب من ٢٫٨٨** — أي
أكثرَ من نصفِ الحزمة، ولا شيءَ منها شيفرة:

- `public/branding/world-pi-logo-*-1024.png` (١٫٢ م.ب) — شعارا ١٠٢٤×١٠٢٤ محضَّران
  **لحقول الرفع** في نماذج التسجيل. التطبيقُ لا يطلب أيًّا منهما.
- `dash-laptop.png` (٣٥٧ ك.ب) — لقطةُ شاشةٍ للتوثيق في جذر المستودع.

**شعارٌ مقاسُه لحقلِ رفعٍ كان سببَ رفضِ الرفع.** بعد استبعادها: **١٬٣٢٩ ك.ب**.

### والتعليقُ المتقادم على الملفّ الذي وظيفتُه قياسُ الأحجام

رأسُ `package.mjs` كان يصف شجرةً لم تعد موجودة: «١١٧ ملفَّ اختبار، ٢٦ وثيقة، تسعةَ
عشرَ لقطةَ ترحيل، والحزمةُ الكاملة ١٫٤ م.ب». الحقيقةُ اليوم ١٩٢ و٢٧ و٢٣ و٣٫٧ م.ب.
**تعليقٌ متقادمٌ عن الأحجام على الملفّ الوحيد الذي يقيس الأحجام** — وهو سببُ أنّ
أحدًا لم يعرف أنّ الحزمةَ توقّفت عن العمل حتى شغّلها إنسان. صُحّح، **وأُضيف
`scripts/package.test.ts`** بتسعةِ اختبارات، منها قاعدةٌ عن الحجم لا عن الملفّات
الثلاثة التي نعرفها: *لا ملفَّ غيرَ شيفرةٍ فوق ١٠٠ ك.ب في حزمة studio* — لأنّ
التالي سيكون ملفًّا آخر.

### والرسالةُ التي كانت تنصح بحذف التطبيق

عند التجاوز كان يقول: «Trim STUDIO_EXCLUDE further». وقد صار هذا **غيرَ صحيح**:
كلُّ ما ليس شيفرةَ تطبيقٍ خارجٌ أصلًا، والباقي `lib/` و`app/` و`components/`
والترحيلات. اتّباعُ النصيحة يعني **شحنَ حزمةٍ ليست هذا التطبيق**. الرسالةُ الآن
تقول الحقيقةَ وتسمّي المسارَ بلا سقف: تطبيقٌ يُصيَّر على الخادم لا يُرفع إلى Pi App
Studio أصلًا — يُسجَّل النطاقُ المستضاف في Pi Developer Portal وتبقى الشيفرةُ حيث
تُخدَم.

### ثانيًا: الدراسة الشاملة — `docs/PROJECT_DOSSIER.md`

١٧ قسمًا، **وكلُّ رقمٍ فيها مقيسٌ من المستودع أو من الشبكة الحيّة**، مستخرَجٌ
باستيراد الوحدات نفسِها لا بقراءة وثيقةٍ قديمة:

٢٤٧ تكاملًا (٢٤٣ صالحًا ترخيصيًّا، **١٦٥ فعّالًا**، ٤٢ محجوزًا، ٢٣٣ بلا مفتاح، ١٩٠
مضيفًا) · **١٨٠ أصلًا مستقلًّا** · ١٤ عائلةَ وصول · ٣٣ بوّابةً في ٧ عائلات · ٨٤
مسارَ API · ٩ صفحات · ٨٧ مكوّنًا · ٢٣ ترحيلًا · ١٩٣ ملفَّ اختبارٍ و**٢٬٦٧٣
اختبارًا** · ٣٣ منافسًا متتبَّعًا · ٧١ بندًا مكتملًا مقابل ٤ مفتوحة.

### وملاحظةُ أمانةٍ وجدتُها أثناء الجمع

حقلُ `publishers` لعائلة **شفافيّة الشهادات** يحمل **١٠ مليار شهادة، لا ١٠ مليار
ناشر**. وجمعُه مع البقيّة يُنتج عنوانًا تسيطر عليه **وحدةُ قياسٍ مختلفة** — وهو
عينُ التضخيم الذي وُجد §٢أ لمنعه، داخل الملفّ المكتوبِ لمنعه.

**ولم أصلحه بصمت ولم أتجاهله:** تحقّقتُ أنّ `catalogSummary()` **غيرُ منادًى من أيّ
صفحةٍ أو مكوّن**، فالرقمُ لا يُعرض لمستخدمٍ اليوم ولا كذبَ حيًّا. سجّلتُه في
الدراسة (§٤٫٣) بتوصيةٍ صريحة: فصلُ الوصول بوحداتِه قبل أن يبلغ أيَّ واجهة. تغييرُ
شكلِ رقمٍ في الميثاق قرارُ منتَجٍ لا أتّخذه وحدي، وإخفاؤه ليس خيارًا.

## R286 — تنبيهُ Supabase: `rls_disabled_in_public`، وقاعدةٌ توقّفت وحدها

> **الطلب حرفيًّا:** «اصلح هذا وايضا التطبيق عل قاعدة البيانات توقف وحده اصلح
> واعمل كل مايلزم وكل الاعدادات والاجراءتت والسياسات والخصوصية داخل قاعدة
> البيانات الان» — مع لقطةِ تنبيهٍ من Supabase لمشروع `roykbyzkskhmzclzobmd`.

### التشخيص: المستودعُ سليم، والقاعدةُ مفتوحة

الـ٢٤ جدولًا كلُّها تحمل `ENABLE ROW LEVEL SECURITY` في `db/schema.sql` وفي
الترحيلات، و`lib/db/rls.test.ts` يُثبت ذلك ويمنع أيَّ جدولٍ جديدٍ بلا حماية.

**وهذا الاختبارُ كان ناجحًا طَوالَ الوقت الذي كانت فيه الجداولُ الأربعةُ والعشرون
مفتوحةً على الشبكة** — لأنّه يقرأ ملفّات. والترحيلُ الذي يُغلقها،
`0022_rls_every_table`، لم يُشغَّل على تلك القاعدة قطّ: النشرُ بلا `DATABASE_URL`،
فـ`npm run db:migrate` لم يعمل هناك أبدًا.

فالشيفرةُ صحيحة، والمجموعةُ خضراء، والانكشافُ حقيقيّ. ولا شيءَ في المستودع يقدر أن
يفرّق بين الثلاثة — إلّا سؤالُ القاعدة نفسِها.

### وما كان المسبارُ يقوله عن تلك القاعدة بالضبط

```
connected to PostgreSQL 15.8 in 42ms; all 24 tables present   →  ok
```

**`ok`** فوق جدول `credentials` يحوي بصماتِ كلماتِ المرور، مقروءًا لكلّ من يحمل
مفتاحَ المشروع العامّ. «كلُّ الجداول موجودة» ليست «كلُّ الجداول محميّة»، والتبليغُ
بالأولى كأنّها تحسم الثانية هو عينُ الخلل الذي تُزيله هذه الجلسةُ منذ بدايتها.

### الانكشافُ مُثبَتٌ عمليًّا، لا موصوفًا

على PostgreSQL 16.13 محلّيّ، بنسخةٍ من مخطَّطنا حُذفت منها كلُّ أسطر RLS:

| | قبل | بعد |
|---|---|---|
| جداولُ مفتوحة | **٢٤ من ٢٤** | **٠** |
| `anon` يقرأ `credentials` | `victim@example.com  argon2id$REAL-HASH` | `permission denied` |
| `anon` يكتب | **أعاد كتابة بصمة كلمة المرور** (`UPDATE 1`) | `permission denied` |
| `anon` يحذف | **أتلف الصفّ** (`DELETE 1`) | `permission denied` |
| دورُ التطبيق (مالك، غير فائق، بلا bypassrls) | يقرأ ويكتب | **يقرأ ويكتب — لم يتغيّر** |

### وخطأٌ في نصّي أنا، أمسكه القياس

كتبتُ في المسوّدة `FORCE ROW LEVEL SECURITY` على `credentials` و
`verification_codes`. ثمّ قِستُه:

```
دورٌ يملك الجداول، غير فائق الصلاحيّة، بلا BYPASSRLS
  SELECT count(*) FROM users        -- RLS عاديّ  ->  2 صفّ
  SELECT count(*) FROM credentials  -- FORCE      ->  0 صفّ
```

**صفرُ صفوفٍ وبلا خطأ.** المالكُ يتجاوز RLS عادةً، و`FORCE` يزيل هذا بالضبط؛ وبلا
سياساتٍ مكتوبة يُمنَع المالكُ من كلّ شيء. أي أنّ قسمي كان سيكسر تسجيلَ الدخول
والتسجيلَ **في صمت** — «لا حساب بهذا البريد» بدل خطأ. حذفتُه ووثّقتُ القياسَ في
الملفّ، لأنّ التهديدَ في التنبيه هو المفتاحُ العامّ، والقسمان ١ و٣ يُغلقانه تمامًا.

### وتحقّقٌ اكتشفتُ أنّه باطل

«بوستجرس عاديّ بلا أدوار Supabase» نجح أوّلَ مرّة — والأدوارُ في بوستجرس **على
مستوى العنقود لا القاعدة**، فـ`anon` كان موجودًا من قاعدةٍ سابقة. أعدتُه بعد حذف
الأدوار فعلًا: صفرُ أخطاء، والدوران يُبلَّغ عن تخطّيهما بالاسم.

### ما بُني

- **`db/ops/rls-remediation.sql`** — إصلاحٌ فوريّ يُلصَق في محرّر SQL. يدور على كلّ
  جدولٍ عاديٍّ في `public` لا على قائمةِ أسماء؛ **مُعادٌ بلا ضرر** (تشغيلٌ ثانٍ:
  صفرُ أخطاء، صفرُ جداولَ أُغلقت)؛ ومقسومٌ إلى معاملتين حتى لا يُلغيَ فشلُ سحبِ
  الامتيازات إغلاقَ الجداول.
- **`lib/db/probe.ts`** — حقلُ `unprotectedTables` يُقرأ من `pg_class.relrowsecurity`
  نفسِه. و`null` ليست `[]`: «لم نسأل» و«سألنا ولا شيءَ مفتوح» جوابان مختلفان.
  والحكمُ صار `off` لا `degraded` — قاعدةٌ مفتوحةٌ ليست قاعدةً متدهورة.
- **`lib/deployment/diagnose.ts`** — يظهر على `/setup` بقراءةٍ **مبنيّة** من جسم
  المسبار لا بمطابقة كلماتٍ في جملة. وقبله كان الانكشافُ يصل كـ«قاعدةُ البيانات غير
  صالحة للاستعمال» — عكسُ الحقيقة، ويُرسل القارئَ إلى إعدادات اتّصالٍ سليمة.
- **١٣ اختبارًا جديدًا** على الحالتين.

### ولماذا «توقّفت وحدها»

مشاريعُ Supabase على الخطّة المجّانيّة **تُوقَف تلقائيًّا بعد نحو سبعة أيّامٍ بلا
نشاط**. ولا شيءَ يتّصل بها: النشرُ بلا `DATABASE_URL`. فهي لم تتعطّل — نامت لأنّ
أحدًا لم يوقظها، وهو العَرَضُ نفسُه لسببِ بقاء الترحيل ٠٠٢٢ غيرَ مُطبَّق.

## R287 — «اصلح كل الأخطاء والثغرات»

مسحٌ أمنيٌّ منهجيّ. النتائجُ النظيفة أوّلًا، فهي جزءٌ من الجواب: **صفرُ ثغراتٍ في
التبعيّات** (`npm audit`)، ماسحُ الأسرار نظيف، **لا `dangerouslySetInnerHTML` في
المشروع كلِّه**، وكلُّ SQL عبر معاملات Drizzle المربوطة. ومسارُ الصور
`avatar/[...path]` محصَّنٌ ضدّ تسلّق المسار بثلاث طبقات، ومقارناتُ الأسرار
`timingSafeEqual`، والمساراتُ الإداريّةُ كلُّها مُبوَّبة، وCSP تحمل
`frame-ancestors 'none'` و`object-src 'none'`.

وثلاثُ ثغراتٍ حقيقيّة:

### ١. الدخولُ بلا سقفٍ — تخمينُ كلماتِ المرور بلا ثمن

`lib/api-scope.ts` يُعفي **كلَّ `/api/auth/`** من حدّ البوّابات، ويكتب سببَه:

> «`/api/auth/*` — الدخول يحتاج سياستَه الأشدّ؛ ٣٠/دقيقة مشتركةٌ فضفاضةٌ لتنفع
> وضيّقةٌ ليكون منتَجًا.»

المنطقُ صحيح. **والسياسةُ لم تُكتب قطّ.** مساراتُ الرموز (`verify/*`،
`password/*`) تحمل `codeRateLimit` فكانت سليمة؛ أمّا `login` و`register`
و`auth/pi` فلم تحمل **شيئًا**: معفاةٌ بالوسيط، وخاليةٌ من حدٍّ خاصّ، على الثلاثة
التي يكون التخمينُ فيها هو الهجوم. تعليقٌ يصف نيّةً كأنّها واقع.

كُتبت السياسةُ في `lib/auth/sign-in-limit.ts` **بمفتاحين لأنّ الهجومين اثنان**:
عنوانٌ واحد يمشي في قائمةِ كلماتِ مرور (١٠/دقيقة)، وحسابٌ واحد يُطرَق من عناوينَ
كثيرة — وهو ما يبدو عليه حشوُ الاعتمادات فعلًا ولا يراه حدُّ الـIP وحده
(٢٠/دقيقة). وحدُّ الحساب **أوسعُ عمدًا**: رقمٌ ضيّقٌ هنا حرمانُ خدمةٍ يُهدى لكلّ
من يعرف بريدَ مستخدم. والمعرّفُ يُجزَّأ في المفتاح ولا يُخزَّن.

### ٢. تزويرُ طلبٍ من جانب الخادم في `/api/mcp`

```ts
const origin = new URL(request.url).origin      // ← يبنيه Next من ترويسة Host
const res = await fetch(`${origin}${gatewayPath(gateway)}`, …)
```

اسمُ البوّابة مُتحقَّقٌ منه والمسارُ ثابت. **والمضيفُ حُرّ** — وهو كلُّ التزوير.
ترويسةٌ واحدة (`169.254.169.254`، أو منفذٌ على العقدة نفسها) تجعل خادمَنا يُصدر
الطلبَ من داخل الشبكة، ويعيد إلى الطالب حتى ٣٠٠ محرفٍ من الجواب عبر `toolError`.

### ٣. تسميمُ روابطِ البريد في `/api/follow`

رابطُ التأكيد كان يُبنى من `x-forwarded-host`. أي أنّ ترويسةً واحدة تجعل خادمَنا
**يُرسل إلى الضحيّة رابطًا على نطاق المهاجم يحمل رمزَ الضحيّة**. والشكلُ نفسُه في
الملفّات المشتركة والمنشورات.

**والسببُ واحد:** ثمانيةُ مواضعَ تبني عنواننا العامّ من `x-forwarded-host` أو
`host` أو `request.url` — وثلاثتُها فكرةُ *الطلب* عمّن نكون، والطلبُ يكتبه مُرسِلُه.
الحافّةُ المُدارة تستبدلها عادةً، ولهذا لم يقع شيء؛ وذلك ضبطُ المنصّة لا ضبطُنا،
والقاعدة #٤ في الميثاق تفرض أن ينتقل هذا إلى مضيفِ Node عاديٍّ بلا إعادة كتابة.

فصار القرارُ في `lib/http/self-origin.ts`: **الإعدادُ يفوز دائمًا**
(`NEXT_PUBLIC_SITE_URL` ← `URL` ← `VERCEL_URL`)، والترويسةُ لا تُصدَّق إلّا إذا
سمّاها المشغّل في `TRUSTED_HOSTS` أو كان المتّصلُ حلقةً محلّيّة. و`null` تعني
«لا نعرف» — و`follow` **يرفض الإرسال** عندها بدل أن يخمّن.

### وإصلاحان أصغر

- **الكوكي كان معرَّفًا مرّتين وتباعدا:** `attachSession` يضع `sameSite` و`secure`،
  وتسجيلُ الخروج يمسح بلا أيٍّ منهما. المسحُ يعمل (المتصفّح يطابق بالاسم والنطاق
  والمسار)، لكنّ تعريفين لكوكيٍّ واحد هو الطريقُ الذي تُنسى به السمةُ التالية —
  على الكوكي الذي هو الجلسة. صار `clearSession` يشاركه التعريف.
- **`RateLimiter` بلا `reset`:** كتبتُ `signInLimiter.reset?.()` في `beforeEach`
  فكان لا شيئًا، وبعضُ اختباراتي كان ينجح **بسبب الحدّ الآخر**. أضفتُ المنفذَ
  وتحقّقتُ بالنقض: تعطيلُ حدّ المتّصل يُسقط **٦** اختبارات، وتعطيلُ حدّ الحساب
  يُسقط **١**.

١٩٦ ملفًّا · ٢٬٧١٨ اختبارًا · الأنواعُ والبناء نظيفان.

## R288 — «افحص كل نقطة مع قاعدة البيانات»

فحصٌ كامل، **على قاعدةٍ حقيقيّة لا على الملفّات**: PostgreSQL 16.13 محلّيّ،
`db/schema.sql` مطبَّقًا، ثمّ استجوابُ فهرس النظام نفسِه.

### النظيف أوّلًا، فهو جزءٌ من الجواب

**٠ دوالّ · ٠ مشغّلات · ٠ مناظر · ٠ `SECURITY DEFINER`** — لا شيء يحمل صلاحيّةَ
المالك متجاوزًا الفحوص. والامتدادُ الوحيد `plpgsql`. وقواعدُ الحذف المتتالي
مصمَّمةٌ بعناية: ما **هو** الشخص يتتالى معه (الاعتمادات، التحقيقات، المراقبات،
مجموعاته)، وما **كتبه** يُفصَل عنه (`SET NULL`) فلا يُسحب من قرّاءٍ آخرين.

### وأربعةُ أعطال

**١. ستّةُ مفاتيحَ أجنبيّةٍ بلا فهرس.** بوستجرس لا يُنشئها تلقائيًّا، والثمنُ في
موضعين: كلُّ ربطٍ عبر العمود مسحٌ تسلسليّ، وكلُّ حذفٍ من الأب يمسح الابن لتطبيق
التتالي. و`entity_links.from_entity_id`/`to_entity_id` **طرفا كلّ حافّةٍ في
الرسم** — «ما الذي يرتبط بهذا الكيان؟» سؤالُ الأنطولوجيا المركزيّ، وكان يقرأ
الجدولَ كلَّه. وُجدت بسؤال الفهرس، لا بقراءة الاستعلامات. الترحيل `0023`.

**٢. الحذفُ لم يكن يحذف.** التعليقُ فوق `repo.users.remove` يقول: «إبقاءُ الاسم
مرتبطًا بهم يُبطل الحذف». قِستُ: أنشأتُ حسابًا، حذفتُه، ثمّ سألتُ عمّا يسمّي هذا
الشخص:
...
`on delete set null` يمسح `visitors.user_id` **ولا يمسح `display_name`**: الصفُّ
انفصل عن الحساب وظلّ يعرّف الشخص — مكتوبًا فوق الشيفرة التي أبقت الاسم. و
`verification_codes` و`email_followers` تُفهرَس بالبريد **بلا مفتاحٍ أجنبيّ إلى
`users` أصلًا**، فلم يصلها شيء: رمزُ تسجيلٍ واشتراكُ نشرةٍ عاشا بعد الحساب.

فصار الحذفُ أربعَ خطواتٍ في **معاملةٍ واحدة** — لأنّ شخصًا محذوفًا نصفَ حذفٍ أسوأُ
من غير المحذوف: قراءةُ عناوينه **قبل** أن يتتالى `credentials` ويأخذ البريد معه،
ثمّ تجهيلُ صفوف الزوّار (العدّادات تبقى، وكلُّ حقلٍ يعرّف يذهب)، ثمّ حذفُ الرموز
والاشتراك، ثمّ حذفُ المستخدم.

**والنتيجةُ مقيسةٌ على القاعدة نفسِها:** `surviving_pii=(none)` مع
`visitor_rows_kept=1 visits_kept=42` — العدّادُ محفوظٌ بلا هويّة، وهو بالضبط ما
يرسمه التعليق. ومُتحقَّقٌ بالنقض: بتعطيل الخطوتين تسقط أربعةُ حقولٍ في وجه الاختبار.

**٣. لا احتفاظَ مجدول.** `verification.sweep()` انتهازيّ — يُستدعى عند إصدار رمز.
أي أنّ نشرًا بلا تسجيلاتٍ لا يكنس أبدًا، فتبقى آخرُ الرموز بعناوينها إلى الأبد.
ورمزٌ منتهٍ لا غرضَ له، وعنوانٌ محفوظٌ بلا غرضٍ هو ما يمنعه §٣. صار
`/api/cron/retention` يوميًّا، بالتنفيذ نفسِه. مُختبَرٌ حيًّا: ٣ رموز → حذف ٢،
وبقي الحيّ وحده.

**٤. ومسحُ الكوكي مكرَّرٌ للمرّة الثانية.** `clearedResponse` في مسار الحساب كان
يكتب السماتِ بنفسه كما كان تسجيلُ الخروج — الخللُ نفسُه في موضعٍ ثانٍ. صار
يستعمل `clearSession`.

### واختبارٌ لا يمكن أن يكون وحدويًّا

العطلُ الثاني كان **خفيًّا على كلّ اختبارات المستودع وسيبقى**: الجملةُ صحيحة،
والتتاليات معلنةٌ صحيحةً، والتعليقُ يصف النيّةَ بدقّة. كلُّ طبقةٍ سليمةٌ وحدها،
والخطأُ في **النتيجة** — ولا يعرفها إلّا بوستجرس. فـ`lib/db/erasure.integration.test.ts`
يعمل على قاعدةٍ حقيقيّة، **ويُتخطّى بلا `TEST_DATABASE_URL`** لا يفشل: مجموعةٌ
حمراءُ عند الجميع مجموعةٌ يتوقّف الناسُ عن قراءتها. والمتغيّرُ ليس `DATABASE_URL`
عمدًا — توجيهُه إلى نشرٍ حقيقيّ يحذف صفوفَه.

ويحمل أيضًا أسئلةَ الفهرس: **لا مفتاحَ أجنبيٍّ بلا فهرس · لا جدولَ بلا RLS · لا
دالّةَ `SECURITY DEFINER`**.

### الحُجّةُ النهائيّة، على قاعدةٍ جديدةٍ من المخطَّط المولَّد

```
tables | rls_off | unindexed_fks | security_definer
    24 |       0 |             0 |                0
```

### قرارٌ للمالك

`visitors` بلا حدِّ احتفاظ. العدّادات مجهَّلةٌ بعد الحذف، لكنّ صفوفَ من لم يُحذف
حسابُه تبقى بلا أجل. تحديدُ المدّة قرارُ منتَجٍ وقانون، لا قرارُ مطوّر — أقترحه
ولم أفرضه.

## R289 — «زيبٌ تحت ميغابايتٍ واحد، وهو الحدُّ المسموح»

الطلبُ نفسُه للمرّة الثالثة، وقد رفضتُه مرّتين بحُجّةٍ صحيحةٍ وناقصة: «ما بقي
شيفرةُ التطبيق، والقصُّ فيها يشحن شيئًا ليس هذا التطبيق». صحيح — لكنّي لم أكن قد
قِستُ **كلَّ** ما يمكن إخراجُه.

### ما قِيس أوّلًا

الضغطُ لا ينقذ: على مجموعة الملفّات نفسِها **deflate ١٣٤٥ ك.ب · bzip2 ١٣٤١ ك.ب**.
والتركيب: `lib` ٢١٤٤ ك.ب · `components` ٧٤٨ · `app` ٣٠٥ · `db` ١٧٦.

ثمّ قِستُ ما لم أفكّر فيه: **التعليقات**. عبر ٤٧٨ ملفَّ TypeScript في الحزمة:
**١٬٠٥٩ ك.ب من ٣٬٣٢١ — ٣٢٪ تعليق.**

وهي أميزُ ما في هذا المستودع: لا تصف الشيفرة، بل تحمل **السبب** — القياسَ خلف
العتبة، والعطلَ الذي كُتب الحارسُ بعده، والمقايضةَ ولماذا. لكنّ حزمةَ رفعٍ غرضُها
أن تعمل لا تحتاجها، وهي **الفئةُ نفسُها** التي أُخرجت بها الاختباراتُ والوثائق:
أدواتٌ حول تطبيقٍ عامل، تُحجب عن **منظرٍ** واحدٍ صغير بينما المستودعُ والحزمةُ
الكاملة يحفظان كلَّ بايت.

```
٢٨٨٢ ك.ب  الحزمة كما كانت، فوق السقف بصمتٍ شهورًا
١٣٤٥ ك.ب  بعد إخراج ثلاثة أصولٍ لا تُحمَّل في أيّ صفحة
 ٨٢٢ ك.ب  بعد تجريد التعليقات من النسخة المرحليّة
```

### ولماذا شجرةُ TypeScript لا تعبيرٌ نمطيّ

تعبيرٌ نمطيٌّ يحذف `//` يحذفها أيضًا من داخل السلاسل والقوالب والتعابير النمطيّة،
فيُنتج حزمةً تنهار في الطرف الآخر بسببٍ لا يربطه أحدٌ هناك بالتحزيم. المُجرِّد
يُحلّل كلَّ ملفٍّ ويُعيد طباعةَ الشجرة بلا تعليقات — فما يُحذف هو ما تسمّيه اللغةُ
تعليقًا. **٠ إخفاقٍ عبر ٤٧٨ ملفًّا**، وملفٌّ يعجز الطابعُ عنه يُنسَخ كما هو.

### والتحقّقُ قبل التسليم

- **الشجرةُ المرحليّة تُفحص أنواعُها** قبل أن يُسمح لها بأن تصير أرشيفًا، والمُحزِّم
  يرمي إن لم تُترجَم. وهذا الفحصُ أمسك خطأً في استدعائي: مرّرتُ `tsconfig`
  المستودع مع `--rootDir` فسحب `tsc` ملفّاتِ `.next/types` من خارج الجذر.
- **وبُنيت الشجرةُ المجرّدةُ ببناء Next كاملًا:** `✓ Compiled successfully in 19.0s`.
  فحصُ الأنواع ليس البناء، والفرقُ هو ما يستحقّ القياس.
- وستّةُ اختباراتٍ تُثبت أنّ المُجرِّد يحذف التعليقاتِ **ولا شيءَ سواها**: علامةٌ
  داخل سلسلة، وداخل قالب، وداخل تعبيرٍ نمطيّ، وJSX، والأنواع، والتصديرات.

**النتيجة: ٨٢٢ ك.ب** من سقف ١٬٠٢٤ — ٢٠٢ ك.ب هامش. والحزمةُ الكاملة ٣٬٨٠٠ ك.ب
بكلّ شيء.

## R275c — طبقةُ الإسناد: العالمُ الدائم تحت الأحداث

### الفجوة

خمسُ طبقاتٍ على اللوحة، وكلُّها **منظرٌ لمجموعة الأحداث**: الأحداثُ والتعاضدُ
والتأخّرُ والتغطيةُ والسيولة. خمستُها تجيب عن «ماذا وصل في هذا المسح»، وخمستُها
تختفي حين يهدأ المسح. **ولا شيءَ على اللوحة يقول أين يقع أيُّ شيء.**

وهذه فجوةٌ حقيقيّةٌ في سطحٍ استخباراتيّ لا زينة: علامةٌ عند `34.05, -118.24` نقطةٌ
قرب ساحل؛ العلامةُ نفسُها بجوار كلمة «Los Angeles» حقيقةٌ يتصرّف بها القارئ.
حدودُ الدول تُشحن أصلًا وتجيب على مقياس الأمّة؛ وهذه تجيب على المقياس الذي يفكّر
به الإنسان.

### المصدر، والعدّ الأمين

Natural Earth 1:50m Populated Places — **ملكيّةٌ عامّة (CC0)**، الناشرُ والرخصةُ
نفسُهما لحدود الدول التي نشحنها. قِستُ المستويين قبل الاختيار: 110m فيه ٢٤٣ مكانًا
جلُّها عواصم بـ٥ ك.ب مضغوطة، و50m فيه **١٢٥١ مكانًا عبر ٢٢٠ دولة بـ٢٣ ك.ب**.
الأوّلُ يجيب «قربَ أيّ دولة»، والثاني «قربَ أيّ مدينة» — وهو السؤالُ الذي يملكه من
ينظر إلى علامة.

### الاختيارُ مكانيٌّ لا سكّانيّ

١٢٥١ تسميةً لا تُرسم. والطريقةُ الساذجة — عتبةُ سكّانٍ عالميّة — تُنتج خريطةً
**خاليةً عبر معظم أفريقيا وغيرَ مقروءةٍ عبر الراين**، لأنّ السكّان موزّعون بتفاوتٍ
ولا عتبةَ واحدةٌ تصحّ في الموضعين.

فالاختيارُ بالمسافة على الشاشة: تُرتَّب الأماكنُ بالأهمّيّة، وتُدمج المتقاربةُ
بصريًّا، ويُسهم كلُّ عنقودٍ بقائده. فيفوز الأهمُّ في المنطقة المزدحمة، وتحتفظ
المنطقةُ الخفيفةُ بمدينةٍ صغيرةٍ كانت العتبةُ ستُسقطها، وتستوي الكثافةُ لأنّها
تُفرَض **بالبكسل لا بالبشر**.

وهذا بالضبط ما يفعله `lib/geo/cluster.ts` للأحداث — بالجشع نفسِه وبترتيب الإدخال
نفسِه وبالحتميّة نفسِها — **فهو الدالّةُ نفسُها لا تنفيذٌ ثانٍ للفكرة** (القاعدة #٦).

### ما قِيس، لا ما رُجّي

- **الحزمة:** البياناتُ تهبط في **جزءٍ منفصل** (١٦٢ ك.ب خام / ٦٢ ك.ب مضغوطًا، مع
  الأطلس والسطح معًا) و**ليست في التحميل الأوّل للجذر** — تحقّقتُ من
  `app-build-manifest.json`. فأوّلُ رسمٍ بلا كلفة، والزيادةُ ٢٣ ك.ب على جزءٍ كان
  يُجلَب أصلًا للأطلس.
- **أربعُ تأكيداتٍ في متصفّحٍ حقيقيّ** على خادمٍ حقيقيّ: تُرسَم افتراضيًّا، ولا
  تتجاوز سقفَ الكثافة، و**تتوقّف فعلًا** عند الإطفاء، وتبقى خفيفةً على الهاتف.
- **ونظرتُ إليها:** ٢٨ تسميةً على محمول — ريكيافيك وهاميلتون ونجامينا في المناطق
  الخفيفة، وباريس وموسكو **وحدهما** في أوروبا. هذا ما بُني الاختيارُ المكانيُّ له.

### والسمةُ التي تجعل هذا قابلًا للفحص

اللوحةُ معتمةٌ على أيّ اختبار — وهو الخلل الذي جعل عشرةَ أحداثٍ بلا إحداثيّةٍ
تُرسم عند `0,0` والشارةُ فوقها تقول «٠ من ١٠». فالسطحُ يعلن `data-labels`: **ما
رسمه الإطارُ الأخير**، لا ما نواه. بدونها «الطبقةُ مشتغلة» و«الطبقةُ رسمت شيئًا»
ملاحظةٌ واحدةٌ من الخارج، والفرقُ بينهما هو الميزةُ كلُّها.

### والمفتاح

`prefs.globe.places` — مفتاحٌ خاصٌّ به لا قيمةٌ سادسةٌ لـ`layer`، لأنّه ليس بديلًا
عن الطبقات الخمس بل يُرسم **تحت** أيّها ظهر. وكم يظهر تقرّره الكثافةُ القائمة لا
مقبضٌ ثانٍ — و`minimal` تعني صفرًا، وهو ما يجب أن تعنيه.
