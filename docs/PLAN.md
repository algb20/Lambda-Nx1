...
- [~] Research & Tech-trend (#27, part 1): OpenAlex + Crossref → `lib/modules/research` + `POST /api/intelligence/research` + "Research" UI mode + tests (papers = claims, graded; most-cited first). Remaining: patents (USPTO/EPO), GitHub/HF trends, macro/economy (World Bank/FRED), and pointing the automated Radar at the ⭐ feeds.
