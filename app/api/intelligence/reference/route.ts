...
import { investigateReference } from '@/lib/modules/reference'
...
export async function POST(request: Request) {
  ...
  try {
    const report = await investigateReference(query)
    // Best-effort history: a failed archive must never cost the result.
    const userId = await getSessionUserId().catch(() => null)
    if (userId) {
      await recordRunSafely({ userId, gateway: 'reference', subject: String(query), report })
    }
