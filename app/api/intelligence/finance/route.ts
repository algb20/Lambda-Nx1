...
import { investigateFinance } from '@/lib/modules/finance'
...
export async function POST(request: Request) {
  ...
  try {
    const report = await investigateFinance(query)
    // Best-effort history: a failed archive must never cost the result.
    const userId = await getSessionUserId().catch(() => null)
    if (userId) {
      await recordRunSafely({ userId, gateway: 'finance', subject: String(query), report })
    }
