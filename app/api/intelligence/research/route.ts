...
import { investigateResearch } from '@/lib/modules/research'
...
export async function POST(request: Request) {
  ...
  try {
    const report = await investigateResearch(query)
    // Best-effort history: a failed archive must never cost the result.
    const userId = await getSessionUserId().catch(() => null)
    if (userId) {
      await recordRunSafely({ userId, gateway: 'research', subject: String(query), report })
    }
