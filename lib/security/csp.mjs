 ...
 */
/**
 * The live price tape's WebSocket origins.
 *
 * Written out here rather than imported from `lib/markets/tape` because this
 * file is `.mjs` and runs in the middleware and the Netlify config, where a
 * TypeScript import is not available. `lib/markets/tape.test.ts` asserts the two
 * lists agree, so the duplication cannot rot silently — which is the only thing
 * that makes a duplicated list acceptable.
 *
 * Without these, the socket is refused by the browser on the deployed site
 * only, with no server-side symptom at all: exactly the class of fault these
 * tests were written for.
 */
const MARKET_TAPE_HOSTS = ['wss://ws-feed.exchange.coinbase.com', 'wss://ws.kraken.com']
